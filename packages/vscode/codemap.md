...
- **Managed-orchestration owner**: the extension host owns one persisted three-slot scheduler, private loopback tool bridge, OpenCode/Cursor child executor, scoped bridge routes, and shutdown ordering. External OpenCode connections do not receive this private runtime.
...
5. Connection, theme, session activity, and safe managed-task events are pushed from extension host to webview; orchestration snapshots and actions return through API-shaped bridge responses.
