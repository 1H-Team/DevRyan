...
export async function spawnManagedOpenCodeServer(
  ...
  const closeChild = async () => {
    ...
    let closed = await waitForClose(2500);
    if (!closed) {
      try {
        if (process.platform !== 'win32' && typeof child.pid === 'number') {
      ...
      closed = await waitForClose(1000);
    }

    if (closed && typeof child.pid === 'number') {
...
export function createOpenCodeManager(
  ...
  async function startInternal(
    ...
    const reapResult = await reapOrphanedManagedOpenCodeProcesses();
    if (reapResult.reaped.length > 0 || reapResult.removed.length > 0) {
      console.log('[OpenCode] Managed process registry cleanup', {
        reaped: reapResult.reaped.length,
        removed: reapResult.removed.length,
        skipped: reapResult.skipped.length,
      });
    }
  ...
  async function setWorkingDirectory(newPath: string): Promise<SetWorkingDirectoryResult> {
    ...
    // For configured (external) URLs there is no managed server to restart.
    if (useConfiguredUrl && configuredApiUrl) {
      return { success: true, path: change.path };
    }

    // DevRyan's managed server is folder-dependent (it chdirs into the working
    // directory and applies a per-project OPENCODE_CONFIG_DIR agent overlay at
    // startup), so re-point it at the chosen folder by restarting. restartInternal
    // preserves workingDirectory, so the overlay re-applies for the new folder.
    if (server) {
      await restart();
    }
