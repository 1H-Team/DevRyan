...
- `sessionTitleRuntime.ts` adapts the shared web title coordinator to the extension host and broadcasts generated-title projections to every webview; `bridge-proxy-runtime.ts` captures accepted non-Cursor provider/model/variant inputs, triggers placeholder recovery after session-list loads, and the global watcher supplies authoritative idle/update/delete events.
- `zenModelCatalogRuntime.ts` owns the extension-host singleton of the shared served-and-zero-cost Zen catalog used by Settings and session titles.
- `opencodeConfig.ts` owns VS Code-side config entity reads/writes, read-only singular/plural plugin-file discovery, OpenCode Slim config/agent override parity, Slim-installed global agent prompt composition, and managed agent runtime overlays so saved user-side agent model defaults, plugin filtering, blocked ambient MCP tombstones, built-in title disablement, and the hidden no-tools `devryan-title` helper apply to the local OpenCode process.
