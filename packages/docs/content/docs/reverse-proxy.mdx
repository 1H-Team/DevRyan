---
title: Reverse Proxy
description: Configure DevRyan correctly behind Nginx, Nginx Proxy Manager, or another reverse proxy.
---

# Reverse Proxy

Use this page if you run DevRyan behind Nginx, Nginx Proxy Manager, Caddy, Cloudflare, or another reverse proxy.

## Before you proxy it

1. Confirm DevRyan works directly first.
...
- Disable gzip on the proxy if DevRyan is already compressing responses.
...
- DevRyan reachable directly on LAN
...
- DevRyan compresses HTTP responses with gzip (threshold 1 KB).
...
SSE streaming routes are excluded from compression by DevRyan, but the CDN may still buffer them. Check your CDN documentation for how to disable buffering on SSE paths.
