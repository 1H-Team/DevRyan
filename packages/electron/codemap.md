...
- **Renderer exit recovery**: `renderer-recovery.mjs` observes each app window,
  including unexpected `clean-exit`, and reloads only that renderer once. The
  budget resets after a loaded document remains stable for a minute. Repeated
  exits, failed reloads, and thirty-second load timeouts offer a native recovery
  dialog. Shutdown, destruction, and superseding navigation fence delayed work;
  the backend is never restarted. View → Reload Window uses Electron's native
