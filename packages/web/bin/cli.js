...
// Refuse to launch a network-exposed (non-loopback) server without UI auth,
// unless the operator explicitly accepts the risk via
// OPENCHAMBER_ALLOW_UNAUTHENTICATED_LAN=true. Closes the accidental
// `--host 0.0.0.0` (or OPENCHAMBER_HOST) unauthenticated-LAN exposure.
function assertAuthenticatedNetworkExposure({ host, uiPassword }) {
  const configured = typeof host === 'string' && host.trim().length > 0
    ? host.trim()
    : (typeof process.env.OPENCHAMBER_HOST === 'string' ? process.env.OPENCHAMBER_HOST.trim() : '');
...
function readProcessCommand(pid, options = {}) {
  ...
  try {
    ...
    const result = spawnSyncImpl('ps', ['-p', String(Math.trunc(pid)), '-o', 'command='], {
      encoding: 'utf8',
      stdio: ['ignore', 'pipe', 'ignore'],
    });
    if (result.status !== 0 || typeof result.stdout !== 'string') {
...
function isDevRyanServerCommand(command) {
  const normalized = normalizeProcessCommand(command);
  if (!normalized) return false;
  const lower = normalized.toLowerCase();
  return /(?:^|[\s"'])\S*\/packages\/web\/server\/index\.js(?:$|[\s"'])/.test(lower)
    || /(?:^|[\s"'])\S*\/@openchamber\/web\/server\/index\.js(?:$|[\s"'])/.test(lower)
    || /(?:^|[\s"'])\S*(?:devryan|openchamber)\S*\/server\/index\.js(?:$|[\s"'])/.test(lower);
}

function isDevRyanForegroundCliCommand(command) {
  const normalized = normalizeProcessCommand(command);
  if (!normalized) return false;
  const lower = normalized.toLowerCase();
  const invokesCli = /(?:^|[\s"'])\S*\/bin\/cli\.js(?:$|[\s"'])/.test(normalized)
    || /(?:^|[\s"'])\S*\/openchamber(?:$|[\s"'])/.test(lower)
    || /(?:^|[\s"'])openchamber(?:$|[\s"'])/.test(lower)
    || /(?:^|[\s"'])\S*\/devryan(?:$|[\s"'])/.test(lower)
    || /(?:^|[\s"'])devryan(?:$|[\s"'])/.test(lower);
  if (!invokesCli) return false;
  return /(?:^|\s)serve(?:$|\s)/.test(lower)
    || /(?:^|\s)--foreground(?:$|\s)/.test(lower)
    || /(?:^|\s)--no-daemon(?:$|\s)/.test(lower);
}

function isDevRyanPidFileCommand(command, options = {}) {
  if (isDevRyanServerCommand(command)) {
    return true;
  }
  if (options.launchMode === 'foreground') {
    return isDevRyanForegroundCliCommand(command);
  }
  return false;
}

async function validatePidFileIdentity(entry, deps = {}) {
  ...
  const processRunning = typeof deps.isProcessRunning === 'function'
    ? deps.isProcessRunning
    : isProcessRunning;
  if (!processRunning(pid)) {
    return { valid: false, reason: 'process-not-running' };
  }

  const fetchInfo = typeof deps.fetchSystemInfoFromPort === 'function'
    ? deps.fetchSystemInfoFromPort
    : fetchSystemInfoFromPort;
  const systemInfo = Number.isFinite(port) && port > 0
    ? await Promise.resolve(fetchInfo(port)).catch(() => null)
    : null;

  if (systemInfo?.runtime === 'desktop') {
    return { valid: false, reason: 'desktop-runtime', systemInfo };
  }

  if (systemInfo?.runtime && Number.isFinite(systemInfo.pid)) {
    if (systemInfo.pid === pid) {
      return { valid: true, source: 'port', systemInfo };
    }
    return { valid: false, reason: 'port-pid-mismatch', systemInfo };
  }

  const readCommand = typeof deps.readProcessCommand === 'function'
    ? deps.readProcessCommand
    : readProcessCommand;
  const command = await Promise.resolve(readCommand(pid)).catch(() => null);
  if (typeof command === 'string' && command.trim()) {
    if (isDevRyanPidFileCommand(command, { launchMode: entry?.launchMode })) {
      return { valid: true, source: 'command', command, systemInfo };
    }
    return { valid: false, reason: 'command-mismatch', command, systemInfo };
  }

  if (systemInfo?.runtime) {
    return { valid: false, reason: 'port-pid-missing', systemInfo };
  }
...
async function discoverRunningInstances() {
  ...
  try {
    ...
    for (const file of pidFiles) {
      ...
      const identity = await validatePidFileIdentity({ port, pid, launchMode });
      if (!identity.valid) {
...
const commands = {
  async serve(options) {
    ...
    if (targetPort !== 0) {
      ...
      const instanceFilePath = await getInstanceFilePath(targetPort);
      const existingPid = readPidFile(pidFilePath);
      if (existingPid) {
        const storedOptions = readInstanceOptions(instanceFilePath);
        const launchMode = storedOptions?.launchMode === 'foreground' ? 'foreground' : 'daemon';
        const identity = await validatePidFileIdentity({ port: targetPort, pid: existingPid, launchMode });
        if (identity.valid) {
