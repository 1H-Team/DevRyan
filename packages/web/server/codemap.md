...
- **Standard-provider title ownership**: `lib/opencode/standard-session-title-runtime.js` schedules guarded, asynchronous titles after successful proxied prompts. It tries the live zero-cost Zen catalog first, then waits for authoritative idle before using the captured provider/model/variant through an isolated hidden helper session. Strict validation, bounded deferred state, startup helper cleanup, and compare-before-update checks keep placeholder, prompt-derived, helper, and stale results out of the visible session list while preserving explicit/manual names.
