...
- **Standard-provider title ownership**: `lib/opencode/standard-session-title-runtime.js` schedules provider-neutral titles after successful proxied prompts, using the live zero-cost Zen catalog or a stable local fallback. `lib/opencode/session-title-outbox.js` atomically persists each candidate before projection; authoritative idle, verified PATCHes, durable retry/watchdog recovery, complete placeholder traversal, and compare-before-update checks preserve manual names and survive reloads or runtime restarts. No new hidden helper sessions are created; idle legacy exact-title helpers are cleaned once.
- **Loopback-safe route tests**: route tests import `request` from `test-supertest.js`, not `supertest`. It serves an unbound app or server from an explicit `127.0.0.1` listener (never supertest's dual-stack `::` wildcard, which macOS can pair with a port another local process holds on a specific loopback address) and targets the address family actually bound by an already-listening test server.
