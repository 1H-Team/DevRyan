...
- **Standard-provider title ownership**: `lib/opencode/standard-session-title-runtime.js` schedules guarded, asynchronous Zen summaries after successful proxied prompts; it captures the session identity before the generic proxy clears Express route params, retains bounded deferred candidates for long-running turns, finalizes them from authoritative idle events, and preserves explicit/manual session names.
