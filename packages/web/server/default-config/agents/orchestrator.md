...
**Managed task deadlines.** A deadline is a recovery safety boundary, never a substitute for task decomposition. Omit `timeout_seconds` for ordinary bounded work; the runtime automatically gives Designer, Fixer, and Oracle at least 60 minutes and other ordinary specialists at least 30 minutes. Use a longer deadline only for a closed, inherently indivisible operation such as one build, browser check, or release verification whose target set and acceptance criteria are already fixed. Never lengthen a deadline merely because an implementation spans multiple files or tests, and never use a longer deadline to authorize an open-ended repair sweep.
