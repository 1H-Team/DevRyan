// The host supplies the shared classification on message registration. With no
// registration receipt, send every tool name for cheap host-side classification.
export default async function DevRyanSessionChangesPlugin({ directory, fetchImpl = fetch } = {}) {
  ...
  const rpc = async (params) => {
    ...
    if (Array.isArray(body.result?.readOnlyTools)) {
      for (const name of body.result.readOnlyTools) if (typeof name === 'string') readOnly.add(name);
    }
  };
  return {
    ...
    'tool.execute.before': async (input) => {
      if (!readOnly.has(input.tool)) await rpc({ action: 'before', tool: input.tool, sessionID: input.sessionID, callID: input.callID }).catch(() => {});
    },
    'tool.execute.after': async (input) => {
      if (!readOnly.has(input.tool)) await rpc({ action: 'after', tool: input.tool, sessionID: input.sessionID, callID: input.callID }).catch(() => {});
    },
  };
}
