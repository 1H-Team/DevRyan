...
export function createSessionExecutionHost(options) {
  ...
  const cursorOwner = () => {
    ...
    if (!cursorOwners.has(binary)) cursorOwners.set(binary, createSessionExecutionOwner({ runtime, launcher: binary,
      verifyLauncher: verifySessionExecutionLauncher, stopSessions: async ({ sessions }) => {
  ...
  const coordinator = createScopedRevertCoordinator({ runtime, executions, openchamberDataDir: options.dataDirectory,
  ...
  const dispatch = async (input) => {
    ...
    const record = await request(`/session/${input.sessionID}/message/${input.messageID}`, input.directory);
    ...
    if (input.action === 'begin') {
      ...
      catch (cause) { await runtime.cancelLease({ directory: input.directory, token: lease.token }); throw cause; }
    }
    if (input.action === 'cancel-before-start') {
      // Only the trusted companion can attest that it never spawned a process.
      await runtime.cancelUnstartedCall(input); return { cancelled: true };
    }
    const lease = await runtime.leaseForCall(input);
  ...
  return { runtime, executions, coordinator, plugin, isConfined, persistCursorRecord, startCursor,
    recover: async () => {
      for (const directory of await runtime.projectDirectories()) {
        try {
          if (!await isConfined({ directory })) continue;
          await coordinator.recover({ directory });
        }
        catch (cause) { options.onDiagnostic?.({ event: 'session_revert', phase: 'recovery_failed', code: cause.code || 'mutation_recovery_required' }); }
      }
    },
    startReadOnly: (input) => startReadOnlySessionExecution({ ...input, launcher: launcher(),
      storage: path.join(options.dataDirectory, 'harness', 'provider-executions'), interactive: true }),
    beforeCursorPrompt: async (input) => { await runtime.assertAdmission(input); return (await session(input)).revert ?? null; },
    drain: async () => { await Promise.all([...cursorOwners.values()].map((owner) => owner.drain())); await runtime.drain(); } };
}
