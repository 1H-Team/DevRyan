...
## Design
- **Native Context Mode worker isolation** (`context-mode-native-hotfix.js`, `context-mode-worker-pool.js`, `context-mode-worker.js`) extends exact-source provisioning without changing databases, tool schemas, routing/capture hooks or permissions. Four project/storage-scoped workers serialize calls, bound queued count/bytes, retain background-process owners, and never replay crashed calls. Bundled helper strings are generated by `scripts/generate-context-mode-worker-sources.mjs` and checked against their source in tests.
...
- **Exclusive title ownership** (`standard-session-title-runtime.js`, `session-title-outbox.js`, `cursor-session-title-runtime.js`, `runtime-agent-overlays.js`) disables OpenCode's built-in title agent in managed runtime overlays. Standard titles use the live zero-cost Zen catalog within an eight-second deadline and a stable local fallback, durably enqueue the candidate before projection, and apply the same authoritative-idle PATCH/read verification lifecycle to every provider. Startup/reconnect/watchdog recovery eventually visits every loaded placeholder with bounded concurrency, manual titles remain authoritative, and only legacy exact-title helper sessions are cleaned up. Cursor retains its separate path.
