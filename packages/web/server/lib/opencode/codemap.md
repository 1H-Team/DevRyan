...
- **Exclusive title ownership** (`standard-session-title-runtime.js`, `cursor-session-title-runtime.js`, `runtime-agent-overlays.js`) disables OpenCode's built-in title agent in managed runtime overlays. Standard titles try Nemotron first, rotate the live zero-cost Zen catalog within a bounded deadline, and fall back immediately to an isolated no-tools helper using the captured session model/variant. Valid results are projected without advancing the parent revision; xAI may persist while busy, revision-sensitive providers wait for idle, bounded recovery repairs recent placeholders, helper sessions are hidden/cleaned, and guarded comparison keeps manual titles authoritative.
...
- **Harness diagnostics and proactive warmup** (`harness-result.js`, `harness-preflight.js`, `harness-tool-manifest.js`, `turn-timing.js`, `agent-runtime-warmup.js`, `project-prewarm-runtime.js`, `xai-tool-catalog-runtime.js`) expose timing/preflight evidence and populate a bounded Grok catalog cache outside prompt dispatch. Missing or unequal evidence preserves the full tool surface.
