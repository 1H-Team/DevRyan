...
- **Exclusive title ownership** (`standard-session-title-runtime.js`, `cursor-session-title-runtime.js`, `runtime-agent-overlays.js`) disables OpenCode's built-in title agent in managed runtime overlays. xAI titles may persist while busy; revision-sensitive and unverified providers wait for idle. A bounded deferred map carries generated titles across the polling deadline and canonical idle events finalize or re-verify them. Guarded comparison keeps manual titles authoritative.
