...
export const applyMeridianHttpHotfix = ({ configDirectory, fs: fsApi = fs,
  ...
  const original = source.replace(IMPORT, '').replace(PATCHED, MERIDIAN_HTTP_SERVER_ORIGINAL).replace(PATCHED_END, ORIGINAL_END);
  ...
  const patched = IMPORT + original.replace(MERIDIAN_HTTP_SERVER_ORIGINAL, PATCHED).replace(ORIGINAL_END, PATCHED_END);
  if (source !== original && source !== patched) return incompatible('Meridian HTTP source contains an incomplete patch');
  let changed = false;
  try {
    for (const [file, content] of [[helper, helperSource], [entry, patched]]) {
      let previous = null;
  ...
  return { ok: true, changed, version: MERIDIAN_HTTP_HOTFIX_VERSION, originalSha256: expectedOriginalSha256,
    sourceSha256: sha256(patched), helperSha256: sha256(helperSource), transport: 'bun-native-request-signal; node-adapter-preserved' };
};
