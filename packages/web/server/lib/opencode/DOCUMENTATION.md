...
- `packages/web/server/lib/opencode/providers.js`: provider source detection, Anthropic OAuth proxy config helpers, and default Cursor provider bootstrap. DevRyan-created Anthropic proxy configs use the reviewed `opencode-with-claude@1.6.18` release; legacy bare entries migrate to it, while explicit user version pins remain authoritative.
- `packages/web/server/lib/opencode/anthropic-oauth-plugin.js`: canonical `opencode-with-claude` package/version matching and migration policy shared by config, quota discovery, and managed-runtime overlays.
- `packages/web/server/lib/opencode/default-plugins.js`: canonical four-item public DevRyan plugin catalog, including pinned Context Mode. It annotates matching user/project registrations and files, preserves effective custom specs/source paths, and keeps internal runtime helpers out of Settings.
...
- `packages/web/server/lib/opencode/user-profile-provisioning.js`: clean-user provisioning for locally managed OpenCode. It merges the sanitized `default-config/user-profile` plugin/default declarations into `~/.config/opencode`, preinstalls the pinned Slim, Claude, and Meridian packages, preserves an explicit user Meridian dependency pin, and safely merges newly managed dependencies into a user-modified package file without discarding custom fields. It also applies the ownership-aware Meridian OpenCode prompt policy before startup, returns prompt-mode diagnostics/warnings, migrates only the exact legacy Meridian defaults once, and fails visibly without overwriting malformed SDK feature JSON. It migrates the old DevRyan-managed bare Claude plugin spec without overriding an explicit user config pin, synchronizes approved agents, skills, and the Slim wrapper through `.openchamber/user-profile-manifest.json`, preserves other user-modified managed files as conflicts, and runs `bun install --ignore-scripts` only when declared dependencies are missing or changed. The packaged profile excludes MCP configuration, Slim `mcps` fields, auth material, generated state, lockfiles, and machine-specific paths. External OpenCode mode skips this mutation path.
...
- `packages/web/server/lib/opencode/turn-timing.js`: in-memory first-turn timing diagnostics for client/proxy marks, Cursor SDK bridge marks, and observed OpenCode session/message/part events. Once a correlated tool call reaches a terminal state, later stale active-state events cannot make it non-final again.
...
- `packages/web/server/lib/opencode/version-policy.js`: Target external OpenCode runtime policy. DevRyan recommends `anomalyco/opencode` v1.18.9 and surfaces the upstream install command. On Unix, managed startup prefers the canonical `~/.opencode/bin/opencode` installer path over PATH shadows, while explicit settings/environment overrides remain authoritative.
...
- `createTurnTimingRuntime(options?)`: creates an in-memory capped timing store. Returned API:
  ...
  - `getRecentTimings({ sessionId?, limit? })`: returns timing marks/durations plus send metadata and diagnostic counters for provider/model/agent/variant, Cursor worker/run/stream timing, repeated text frames, malformed tool-call diagnostics, loop-guard diagnostics, mutation evidence, and a capped ordered `diagnostics.toolCalls` projection. Tool calls are correlated internally by authoritative call/part identity and repeated states update one entry in place; terminal finality is monotonic if a stale active event arrives later. The response contains only `ordinal`, normalized `tool`, normalized `status`, and `final`, with a default cap of 20 and no identity/input/output fields.
