...
## Bounded environment and executable discovery

`env-runtime.js` keeps its synchronous API and existing explicit/canonical/PATH/fallback precedence. Each environment capture or executable lookup owns a probe runner whose ten-second monotonic deadline begins at its first subprocess. Candidate shells are deduplicated in order. Previously unbounded probes have a five-second ceiling; WSL's existing six-second command ceiling is constrained by the enclosing lookup's remaining budget. Exhausted lookups skip further subprocesses; Node/Bun launch fallback does not repeat an unsuccessful lookup.

Every probe uses `SIGKILL` for timeout termination and rejects execution errors, signals, nonzero exits, malformed output and partial timeout stdout. Environment values and captured shell output are never logged. The internal factory supports injected execution, clock, shell candidates and executable checks; short deadline overrides are test-only dependencies, not settings. `env-runtime.test.js` uses deterministic fakes for host-specific paths plus an isolated short-deadline process that ignores SIGTERM and is verified dead after timeout. Windows and WSL behavior is simulated on non-Windows hosts.

## Entrypoints and structure
- `packages/web/server/lib/opencode/index.js`: public re-export boundary for configuration, agents, skills, provider auth, UI auth, and managed plugin setup.
...
- `packages/web/server/lib/opencode/standard-session-title-runtime.js`: Provider-neutral title coordination for non-Cursor sessions. It keeps the prompt preview while an isolated no-tools helper on the selected session provider/model resolves one title, with a ten-second budget and one delayed retry. All providers follow this lifecycle. Only after both attempts fail does a local summary become the final fallback; no intermediate derived title is projected. Procedural approved-plan prefixes are excluded from the final name. Generic `Managed <agent> Task` child titles are eligible only when canonical parent/agent metadata matches; known initial agent-contract, Context Mode, and read-only prefix blocks are excluded from the task brief. Existing placeholder children recover through session updates or list/watchdog reconciliation, including after restart. Candidates are durably recorded before projection, then PATCHed only after canonical idle or completed-turn recovery; a verifying read retires the job. Custom labels and manual renames remain authoritative.
...
- `packages/web/server/lib/opencode/claude-cli-runtime.js`: shared read-only Claude Code executable resolution for web/Electron. Explicit `CLAUDE_CODE_CLI` wins, followed by the provisioned managed OpenCode package and the host's effective PATH; candidates are validated before execution and no refresh path installs or mutates the runtime.
...
- The managed user-profile registers the document reader and pins `@opencode-ai/plugin@1.18.31`, `adm-zip@0.6.0`, `mammoth@1.12.1`, and `unpdf@1.8.0`. These packages use the same conflict-preserving provisioning and exact installed-entrypoint validation as package-backed managed plugins; configured external runtimes are never mutated.
...
- `packages/web/server/lib/opencode/claude-runtime-compatibility.js`: owns the exact `opencode-with-claude`/Meridian/Agent SDK/Claude Code control and candidate tuples, managed dependencies and Bun overrides, installed-version inspection, and the `.openchamber/claude-runtime-compatibility.json` provenance marker. Provisioning upgrades fields it previously managed (including manifest-owned pre-marker profiles), preserves explicit user overrides, and reports `validated`, `user_managed`, `drifted`, or `upstream_blocked` without reading credentials. It also compares the installed tuple before deciding whether installation is necessary, so a current manifest with stale `node_modules` is repaired; managed startup fails explicitly if installation returns success without converging to the reviewed tuple. The isolated 2026-08-20 gate measured Claude Code `2.1.98` at 49,152 cached-prefix tokens versus 64,815 for the historical `2.1.215` control, and Meridian `1.62.6` requires Claude Code `^2.1.198`. The selected managed tuple uses Claude Code `2.1.251` to satisfy Claude Fable 5.1's minimum version, with `opencode-with-claude` `1.8.0`, Meridian `1.62.6`, and Agent SDK `0.2.141`. Compatibility remains honestly reported as `upstream_blocked` until the broader cross-provider target passes.
- `packages/web/server/lib/opencode/meridian-http-hotfix.js` / `meridian-http-server.js`: the managed Meridian `1.62.6` source/hash gate replaces only its HTTP server entrypoint with native `Bun.serve` when running inside Bun, and keeps the existing Node adapter elsewhere. The bundled Node HTTP adapter under Bun `1.3.14` loses client-disconnect events, allowing cancelled Claude turns to continue and commit while the next turn waits. The native server passes the original Request signal into the unchanged Hono app, request cancellation, SDK abort, queue lease, and stale-history guard. It retains the server lifecycle surface needed by proxy startup and graceful/forced shutdown. Provisioning fails explicitly for incompatible managed source and leaves explicit user-managed tuples untouched; it never rewrites session history, retries a provider turn, or weakens the conflict guard.
- `packages/web/server/lib/opencode/cli-options.js`: CLI/environment option parsing for server startup arguments.
...
- `packages/web/server/lib/opencode/version-policy.js`: Target external OpenCode runtime policy. DevRyan recommends `anomalyco/opencode` v1.18.31 and surfaces the upstream install command. On Unix, managed startup prefers the canonical `~/.opencode/bin/opencode` installer path over PATH shadows, while explicit settings/environment overrides remain authoritative.
...
The Slim adapter loads the exact installed entrypoint and accepts either a legacy plugin factory or Slim 2.2.18's default descriptor with a callable `server`. It invokes only that server factory, preserves runtime hooks, and removes only the existing agent/system-transform ownership while restoring DevRyan's agent and default-agent config after Slim's config hook. Unsupported exports remain visible startup errors. Tauri retains its pinned legacy loader.
...
Web/Electron overlays both inventory the packaged runtime plugins, including the always-on `devryan-skill-context.mjs` adapter; maintenance-only Tauri assets intentionally remain frozen without it. The `devryan-tool-input-guard.mjs` hook rejects known binary paths before native `read` or `oc_read` executes, replaces unexpected signature/control-character binary results before persistence, and removes historical binary read payloads only from provider-bound request copies. Proper image attachment parts, SVG, ordinary UTF-8/Unicode text, unrelated tool output, and canonical historical records remain unchanged. Binary input rejections retain `DEVRYAN_TOOL_INPUT_INVALID`; sanitized results use the bounded `DEVRYAN_BINARY_READ_BLOCKED` notice. The same hook rejects a `grep.path` containing multiple absolute targets and syntax-invalid JavaScript passed to `ctx_execute`, never splits paths, executes validation source, or repairs code. It also validates absolute paths for `read`/`oc_read`, `edit`/`multiedit`/`patch`/`write`, `grep`/`glob`/`list`, and `bash`/`shell` `cwd`/`workdir` before OpenCode mints its `external_directory` prompt: a degenerate path (over 512 characters, over 40 segments, a segment over 255 bytes, a segment repeated four times, or `//` after a non-existent prefix) or an outside-project path under a directory that does not exist is rejected with `DEVRYAN_TOOL_INPUT_INVALID` naming the deepest existing ancestor, so the model self-corrects instead of the user approving a directory that is not there. Relative paths, non-strings, paths inside the project, existing paths, and writes whose external parent exists are untouched, so OpenCode's own prompt still fires for real external paths. Externally managed OpenCode servers remain responsible for their own tool execution policy.
...
unmanaged hosts), direct independent auth-file writers,
...
Native Context Mode execution is gated by the exact 1.0.169 adapter/executor
...
`context-mode-worker-process.js` uses independent JavaScript processes, avoiding
Bun 1.3.14's shared SQLite handle registry and containing native crashes. The
adapter passes its detected runtime configuration so new workers do not repeat
executable discovery. IPC carries tool requests, results and bounded lifecycle
events; a lost parent causes a worker to stop its own command process groups.
There is no active-worker cap, pending-command queue, queue deadline or queued
input-byte allowance. Each worker runs one foreground handler at a time. The
pool retains at most four idle workers, each for thirty seconds; active handlers
and owned background processes are excluded from that idle allowance. Retiring
workers drain normally after acknowledging cleanup, with a watchdog for stuck
exits, and never delay new calls. The constructor accepts `maxIdleWorkers` and
`idleTimeoutMs` instead of the former active/queue limits.

Dispatch starts a 120-second default budget. Positive finite explicit command
timeouts remain supported, including timers beyond the Node/Bun integer range.
Commands receive the same budget, and parallel batch jobs share its remaining
time. The parent watchdog covers initialization, indexing and result delivery;
execution tools receive five seconds for executor cleanup/result delivery.
A timeout with partial command output is an error, never successful completion.
Its stable deadline prefix prevents incidental SQLite text in partial output
from activating host-wide database recovery.

`context-mode-worker-state.js` owns per-database lock identities and
per-session statistics. Database identity uses the canonical on-disk path, not
session identity or the worker cache key. `context-mode-worker-storage.js` parks
only a contending worker using cancellable, deadline-aware waits. Atomic hard
links from unique owner tokens elect each storage lock holder. The parent may
clear an abandoned link only after the owning process exits and its file identity
matches; it never clears a sibling's lock. Locks live in a private disposable
runtime directory, and cancellation markers interrupt synchronous waits. Schema
initialization and stale-row maintenance are coalesced; SQLite WAL readers remain
independent. Periodic full-index defragmentation is omitted from native tool
completion and worker retirement. Only short storage operations and atomic transactions hold a mutex;
file traversal, parsing, commands and fetching remain outside it. Source
replacement keeps the existing last-committed-write semantics. Known rolled-back
transactions may retry contention; complete handlers and post-commit operations
are never replayed. Native recovery reopens a poisoned handle for the next call
without replaying the failed operation or deleting/renaming live database files.
A confirmed worker exit releases any interrupted mutex; cancellation alone does
not. A reused worker invalidates its fuzzy cache after another connection writes.

Workers send one statistics delta per call before its terminal result. The parent
aggregates by initiating session, coalesces private sidecar writes, uses unique
temporary names and restores counters after retirement/restart. Idle statistics
entries have a bounded cache. Ordinary tool responses no longer run lifetime
scans or each write an independent statistics snapshot; explicit `ctx_stats`
queries retain their analytics behavior. SessionDB bookkeeping uses the initiating
session rather than whichever session was most recently active.

Abort remains attached after dispatch. Cancellation/timeouts settle the caller
once and fence only its worker until cleanup is proven; same-project siblings
continue. Active outcomes are unknown and are never replayed. Every child process
carries its worker-call owner, including Rust compilation. Cooperative cancellation
stops only that call's processes and preserves unrelated successful background
work. After five seconds an unresponsive worker without unrelated background work
is terminated. Missing exit/process-tree cleanup proof quarantines that slot,
not its project. Exit/process observations can recover quarantined slots. Exited
slots with remaining groups use a bounded one-second probe cadence; zombie-only
Bun groups are settled but may await runtime reaping. No busy-host restart occurs.

Idle retirement bounds close acknowledgement and termination receipt to five
seconds each. Close releases the existing database without synchronous FTS
optimization or a truncating WAL checkpoint. Shutdown/crash cleanup remains
responsible for all owned children.

`context-mode-diagnostics.js` validates the private `context_mode_diagnostic` RPC
before appending bounded `context_mode.*` lifecycle events. Phases distinguish
worker start/reuse/retirement, dispatch/initialization/execution, storage
contention, completion/error, active timeout/cancellation, unavailability,
quarantine and recovery. Legacy queue phases remain readable for older managed
runtimes but new dispatch emits none. Payloads retain session/message identity,
optional caller call ID, worker-call UUID, per-call sequence, source timestamp,
elapsed time and budget through journal sanitization. Native ToolContext lacks a
call ID, so the adapter stamps `contextModeWorkerCallID` in tool metadata and
passes its verified message ID. No arguments, commands, paths or arbitrary error
text travel on this bridge. Delivery has a bounded 128-event telemetry buffer
and two-second RPC deadlines; the next successful delivery reports dropped events
as `context_mode.diagnostics_gap`. Telemetry never blocks tool execution. A stopped
host can still have unavailable final telemetry; terminal tool parts remain
independent evidence.

`context-mode-worker-sources.js` embeds the pool, process, worker, state and storage helpers
for provisioning. Run `node scripts/generate-context-mode-worker-sources.mjs`
after editing a helper; tests reject stale generated sources. Externally managed
runtimes remain untouched. Normal managed provisioning activates updates after
active work finishes; do not patch/restart a running user's runtime for rollout.
...
OpenCode; readiness waits consume the same remaining budget in web.
...
- `registerOpenChamberRoutes(app, dependencies)`: registers OpenChamber endpoints:
  ...
  - `GET /api/zen/models`
- Update checks await asynchronous package-manager probes so background polling
  cannot block prompt admission, recovery RPCs, or SSE on the shared host event
  loop. Selection preserves forced-manager, installation-ownership, environment
  hint, runtime hint, and visible-package fallback precedence. Availability and
  metadata probes retain their 5-second and 10-second limits and 1 MiB output
  bound. Concurrent checks share one lookup; a resolved package-manager choice
  is cached. Desktop continues to report Electron ownership without probing.
  CLI detection and update execution keep their synchronous contracts.
...
## Plugin upgrades and preserved agent state

Follow [the upgrade runbook](../../../../../docs/PLUGIN_UPGRADES.md) and retain a dated audit. Slim 2.2.18 captures the final host-agent map for model admission, so the adapter restores that map in place even when its config hook fails. Untracked personal presets/prompts are preserved. A conflicting user-owned Slim adapter stops dependency installation with `DEVRYAN_SLIM_ADAPTER_CONFLICT` until reconciled. The Meridian installer has exact source descriptors for the current 1.62.6 and held 1.68.0 candidate; `meridian-upgrade-patches.js` preserves the candidate's upstream fork/publication logic. This does not change the selected Claude tuple or its caching improvements.

## Image generation model policy

`imagegen-model-hotfix.js` patches the reviewed image plugin executable to request `gpt-6-astra` with `reasoning: { effort: "medium" }`. Managed provisioning applies the patch after dependency validation. It verifies the complete original SHA-256, accepts only the complete original or patched source, and writes via atomic replacement to preserve package-manager cache hard links. An incompatible source blocks startup without replacing that source. Bot builds apply the same helper. The image tool schema, OAuth handling, references, quality and file output remain upstream-owned.

### Session title deadlines and diagnostics

Standard-provider titles use the selected model with a 30-second shared budget for helper creation, generation, and optional repair. A completed-answer read then has its own 2.5-second recovery budget. Only finished assistant records qualify for recovery. Helper deletion runs separately with the existing bounded OpenCode request timeout; shutdown cancels active generation and drains cleanup. Transports that ignore cancellation cannot publish late answers. Helpers whose creation response was lost remain eligible for the existing stale-helper sweep.
