...
- `packages/web/server/lib/opencode/harness-preflight.js`: read-only harness preflight diagnostics for agents, skills, MCP/tool manifest state, forbidden managed-runtime tool/MCP exposure, latest warmup state, and packaged prompt context-budget audit.
...
- `packages/web/server/lib/opencode/version-policy.js`: Target external OpenCode runtime policy. DevRyan recommends `anomalyco/opencode` v1.18.3 and surfaces the upstream install command. On Unix, managed startup prefers the canonical `~/.opencode/bin/opencode` installer path over PATH shadows, while explicit settings/environment overrides remain authoritative.
