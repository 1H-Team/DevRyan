...
- `packages/web/server/lib/opencode/version-policy.js`: Target external OpenCode runtime policy. DevRyan recommends `anomalyco/opencode` v1.18.19 and surfaces the upstream install command. On Unix, managed startup prefers the canonical `~/.opencode/bin/opencode` installer path over PATH shadows, while explicit settings/environment overrides remain authoritative.
...
- Accepted non-Cursor `POST /api/session/:sessionID/prompt_async` requests schedule non-blocking standard-provider title generation. The route captures the session ID before handing control to the generic proxy, because Express route parameters are cleared after `next()`. Generated titles are persisted only after authoritative `/session/status` reports the target idle; this prevents a metadata PATCH from advancing Claude/Meridian session state while the provider request is still waiting. Only empty, untitled, or generated-timestamp titles are eligible; custom and historical raw-prompt titles are left untouched, and failures remain retryable on a later prompt.
