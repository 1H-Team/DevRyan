...
- `packages/web/server/lib/opencode/standard-session-title-runtime.js`: Provider-aware title coordination for proxied non-Cursor prompts. It summarizes the earliest visible user text by trying `nemotron-3.5-lightning-free` first and then every currently served zero-cost Zen model within an eight-second phase (4.5 seconds per request, 32 output tokens, no same-model retry). Unavailable/auth/rate-limited models cool down for five minutes and transport/timeout/5xx failures for 30 seconds. The topic-first contract names the durable subject or outcome instead of Plan mode or a requested planning deliverable; a source-aware local correction removes incidental leading planning phrases without another generation call and preserves literal Plan concepts. Strict validation rejects placeholders, plan markers, markdown, raw prompts, and the former 80-character prompt fallback. The source request is JSON-encoded and isolated as untrusted data. If the free pool is exhausted, the runtime immediately creates an exact-title hidden helper session in the same directory, invokes the captured provider/model/variant through the no-tools `devryan-title` agent inside one 45-second deadline, validates the result, and makes one bounded repair request when the first answer violates the title contract. A short pre-cleanup message read recovers valid text that reached the helper session just as the request transport timed out; the helper is then deleted in `finally`. Every valid title is projected to connected UI clients before persistence without changing the parent revision or its `time.updated`; xAI may still PATCH while busy, while revision-sensitive providers retain idle-before-PATCH protection. Bounded deferred state survives long turns, canonical idle events finalize/re-verify titles, and session-list reconciliation re-projects cached results or recovers the 20 newest placeholders. Final comparison preserves manual titles.
...
- The managed user-profile registers the document reader and pins `@opencode-ai/plugin@1.18.23`, `adm-zip@0.6.0`, `mammoth@1.12.1`, and `unpdf@1.8.0`. These packages use the same conflict-preserving provisioning and exact installed-entrypoint validation as package-backed managed plugins; configured external runtimes are never mutated.
...
- `packages/web/server/lib/opencode/version-policy.js`: Target external OpenCode runtime policy. DevRyan recommends `anomalyco/opencode` v1.18.23 and surfaces the upstream install command. On Unix, managed startup prefers the canonical `~/.opencode/bin/opencode` installer path over PATH shadows, while explicit settings/environment overrides remain authoritative.
...
- Accepted non-Cursor `POST /api/session/:sessionID/prompt_async` requests schedule non-blocking title generation with the provider ID, model ID, variant, directory, and visible non-synthetic text captured at acceptance. A valid free-Zen or isolated selected-model result is published immediately as a synthetic `session.updated` projection; xAI may also PATCH while busy, while other providers wait for authoritative idle before persistence. The production title runtime consumes canonical OpenCode events so long turns retain generated candidates after the 120-second poll, `session.status: idle` completes the guarded PATCH or xAI re-verification, and `session.deleted` clears pending work. Successful session-list requests trigger bounded placeholder recovery. This changes no public HTTP contract. Cached schema-verified Grok duplicate-tool disables merge synchronously, while discovery and refresh never block the prompt.
...
Managed runtime directory allowances include POSIX `/tmp` and its canonical real path, alongside the active project, worktree root, and plans directory. They are merged only into agents that already declare a permission block, preserving role-level tool restrictions.
