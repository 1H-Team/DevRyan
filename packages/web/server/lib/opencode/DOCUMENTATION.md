...
- `packages/web/server/lib/opencode/default-plugins.js`: canonical three-item public DevRyan plugin catalog. It annotates matching user/project registrations and files, preserves effective custom specs/source paths, and keeps internal runtime helpers out of Settings.
...
- `packages/web/server/lib/opencode/runtime-surface-policy.js`: central managed-runtime tool-surface policy for allowed plugin specs, blocked MCP tombstones, and forbidden live tool IDs used by overlay generation and harness preflight diagnostics.
...
- `packages/web/server/lib/opencode/harness-preflight.js`: read-only harness preflight diagnostics for agents, skills, MCP/tool manifest state, effective prompt-policy-aware forbidden managed-runtime tool/MCP exposure, latest warmup state, and packaged prompt context-budget audit.
...
- `packages/web/server/lib/opencode/version-policy.js`: Target external OpenCode runtime policy. DevRyan recommends `anomalyco/opencode` v1.18.5 and surfaces the upstream install command. On Unix, managed startup prefers the canonical `~/.opencode/bin/opencode` installer path over PATH shadows, while explicit settings/environment overrides remain authoritative.
...
- `syncRuntimeAgentOverlays(options?)`: synchronizes user-scoped agent model/variant overrides, packaged-agent visible-skill policy, active project/worktree external-directory allows, allowlisted active plugin registrations, bundled runtime plugin files/registrations (including the GitHub Copilot Auto/picker-fallback and OpenAI GPT-5.6 variant plugins), Anthropic OAuth proxy bootstrap config, Cursor provider bootstrap config, GitHub Copilot auth alias synchronization plus account-discovered `github-copilot` provider models with runnable `api` metadata (using `auth.readAuthFile` when callers omit an explicit reader), runtime-only user remote MCP timeouts, bounded active-OpenAI header/chunk/total request timeouts, and blocked ambient MCP tombstones into a managed high-precedence OpenCode config directory. It preserves explicit agent model and OpenAI timeout overrides without synthesizing OpenAI models or availability.
