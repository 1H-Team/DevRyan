...
- `packages/web/server/lib/opencode/standard-session-title-runtime.js`: Provider-aware title coordination for proxied non-Cursor prompts. It summarizes the earliest visible user text, clamps unavailable-model fallback copy to seven words, lets xAI persist while busy, and retains idle-before-PATCH protection for Anthropic and unverified providers. Generated candidates that outlive the bounded status poll remain in a 500-entry, 24-hour deferred map; canonical `session.status: idle` events finalize or re-verify them, while deletion, expiry, and real title updates clear them. Final comparison preserves manual titles.
...
- `packages/web/server/lib/opencode/version-policy.js`: Target external OpenCode runtime policy. DevRyan recommends `anomalyco/opencode` v1.18.21 and surfaces the upstream install command. On Unix, managed startup prefers the canonical `~/.opencode/bin/opencode` installer path over PATH shadows, while explicit settings/environment overrides remain authoritative.
...
- Accepted non-Cursor `POST /api/session/:sessionID/prompt_async` requests schedule non-blocking title generation with the provider ID. xAI may PATCH after generation while busy; other providers wait for authoritative idle. The production title runtime also consumes canonical OpenCode events: long turns retain their generated candidate after the 120-second polling deadline, `session.status: idle` completes the guarded PATCH or xAI re-verification, and `session.deleted` clears pending work. Cached schema-verified Grok duplicate-tool disables merge synchronously, while discovery and refresh never block the prompt.
