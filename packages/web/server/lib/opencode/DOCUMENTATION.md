...
- `packages/web/server/lib/opencode/harness-context-budget.js`: report-only UTF-8 byte accounting for parsed packaged agent prompt bodies, visible skill catalog name/description values, visible on-demand skill bodies, and the live tool ID/description/parameter catalog. It uses the shared canonical-path skill visibility policy, reports packaged prompt provenance (`prompt`, `body`, or `contentFallback`), and labels tool measurements `runtimeCatalogUpperBound`. Duplicate occurrences remain counted and exact duplicate IDs/schemas are reported without claiming request-token counts or returning prompt/skill body content; unavailable or unrequested sources use `null` for counts and duplicate reports instead of claiming an empty successful source.
- `packages/web/server/lib/opencode/cli-options.js`: CLI/environment option parsing for server startup arguments.
...
- `packages/web/server/lib/opencode/version-policy.js`: Target external OpenCode runtime policy. DevRyan recommends `anomalyco/opencode` v1.18.13 and surfaces the upstream install command. On Unix, managed startup prefers the canonical `~/.opencode/bin/opencode` installer path over PATH shadows, while explicit settings/environment overrides remain authoritative.
...
- `buildHarnessContextBudget(options?)`: builds byte-only packaged prompt, canonical-path-visible skill metadata/body, and runtime tool-catalog measurements. Parsed `prompt` is authoritative over full-file `content`, and each packaged prompt measurement records its source. Skill body read failures are reported per item; raw prompt and skill body text is not included in the result. Missing live sources are explicitly unavailable, with `null` counts and duplicate reports.
