...
- The managed user-profile registers the document reader and pins `@opencode-ai/plugin@1.18.27`, `adm-zip@0.6.0`, `mammoth@1.12.1`, and `unpdf@1.8.0`. These packages use the same conflict-preserving provisioning and exact installed-entrypoint validation as package-backed managed plugins; configured external runtimes are never mutated.
...
- `packages/web/server/lib/opencode/version-policy.js`: Target external OpenCode runtime policy. DevRyan recommends `anomalyco/opencode` v1.18.27 and surfaces the upstream install command. On Unix, managed startup prefers the canonical `~/.opencode/bin/opencode` installer path over PATH shadows, while explicit settings/environment overrides remain authoritative.
