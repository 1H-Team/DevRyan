...
- `POST /api/git/commit-message/draft`: Accept `{ selectedFiles, stagedOnly, guidance?, zenModel? }`, collect authoritative Git context in the host, and return the shared workflow result `{ status, commits, message?, warnings? }`. Status and six recent subjects are loaded concurrently; history failure is disclosed without discarding readable changes. Up to 200 unique validated paths use batched tracked diffs and individual untracked-file diffs, with at most two concurrent Git reads and a 16,000-character combined patch budget. Untracked content is included only for unstaged scope, without staging files.
...
- Both routes resolve the live zero-cost Zen catalog, with the last known snapshot on catalog failure. A requested model is preferred only when present in that catalog. Shared rotation tries up to three distinct models, prioritizing warm models and filling remaining slots with cooling models so manual generation can recover. Failures and invalid output advance immediately; each attempt has a 15-second total timeout and aborts its transport before advancing. Provider attempts can take up to 45 seconds, separate from bounded Git context, catalog collection, and helper cleanup. Zen rejects standalone free-tier HTTP inference; production uses `zen-text.js` to call the native OpenCode `opencode` provider in temporary, tool-denied `devryan-commit` / `devryan-pr` helper sessions. Helpers are hidden from navigation and notifications and aborted/deleted before rotation advances. The caller's chat session is never prompted or switched.
- The compact JSON response contains a Conventional Commit subject and two to four details. Native OpenCode owns provider-specific inference options; the retained injectable standalone transport uses 220 Chat Completions / 256 Responses output-token caps for compatibility tests.
- Shared runtime policy repairs otherwise-valid overlong subjects at a word boundary. Only after all selected attempts fail (or no catalog models are available), generation produces a deterministic factual draft derived from selected paths, statuses, and line statistics. Responses disclose fallback or partial context through `warnings` while still returning a usable draft.
- Both routes are read-only for managed mutation-lock purposes, while authentication, CSRF, workspace containment, and Git policy checks still apply. Sanitized phase timings and per-model attempts (model, sequence, duration, HTTP status, and failure category) are written to the diagnostic journal, and web responses expose context/model/provider/parsing durations through `Server-Timing`.
...
- The route adds bounded base/head diff context to the rendered prompt and uses the same up-to-three-model free Zen rotation as commit generation, with 15 seconds per attempt and a 45-second free-tier deadline. It accepts normalized title/body JSON or a usable plain-text draft.
- After free Zen exhaustion or catalog unavailability, the route tries the directory's Builder model, or the client's selected model if no Builder model is available, through an isolated hidden `devryan-pr` helper session with a 60-second budget. The UI HTTP request budget is 150 seconds to cover context, catalog, both tiers, bounded helper cleanup, and response delivery.
- If neither tier produces a usable draft, the route returns a classified error and sanitized attempts without changing the caller's existing draft fields. Successful responses include the selected source and model. Generation does not publish a PR.
