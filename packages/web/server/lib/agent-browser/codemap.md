...
- In a confined execution worker the daemon socket lives under the execution's short private `XDG_RUNTIME_DIR`, which the plugin's environment allowlist passes through, so it stays within the 104-byte macOS limit. The plugin itself is byte-pinned by the qualified duplicate-output profiles, so this is owned by `@openchamber/harness-runtime`.
- Confined commands cannot keep a dev server alive after they end, so the no-preview guidance tells the agent to find an already-running local site, or else ask the user to start one and report verification as blocked.
...
- The bundled skill distinguishes ordinary agent cookie sharing, owner/origin branch-preview partitions, and host/principal manual-browser partitions; it does not imply isolated login state from an isolated tab.
