...
## Design
- `runtime.js` is core engine: next-run computation (`daily/weekly/once/cron`), concurrency limits, jitter, run lifecycle/state, live owner/grant resolution, and ownership-before-prompt execution.
...
3. Scheduler fires due tasks, resolves the owner’s current branch target, starts an OpenCode session, records ownership, and only then submits the command/prompt.
