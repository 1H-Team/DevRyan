...
const normalizeHeaderAsciiFileName = (fileName) => {
...
const formatContentDispositionAttachment = (fileName) => {
  const fallback = normalizeHeaderAsciiFileName(fileName);
...
export const registerFsRoutes = (app, dependencies) => {
  ...
  app.get('/api/fs/stat', async (req, res) => {
    ...
    const optionalMiss = () => res.json({
      path: normalizeDirectoryPath(filePath) || filePath,
      exists: false,
      isFile: false,
      size: 0,
    });
  ...
  app.get('/api/fs/raw', async (req, res) => {
    ...
    try {
      ...
      if (download) {
        ...
        res.setHeader('Content-Disposition', formatContentDispositionAttachment(fileName));
      }
