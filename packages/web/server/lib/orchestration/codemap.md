...
- `runtime.js`: composes one scheduler, acquires cross-process persistence ownership before bridge/recovery startup, suppresses non-owner recovery, safely projects ownership conflicts, handles validated 25-second maximum wait slices, barrier inspection and confirmed agent handoff, external-runtime gating, event publication, and exact-owner shutdown.
- `atomic-ledger.js`: private atomic JSON persistence with an exclusive heartbeat owner lock, dead-process-only stale recovery, per-operation token fencing, legacy dispatch-group hydration, and corrupt-ledger quarantine.
