...
## Responsibility
Monorepo package boundary for DevRyan runtimes. It organizes shared UI/runtime implementations plus shell-specific hosts (Electron primary, Tauri legacy).
...
- **Workspace segmentation by runtime**:
  ...
  - `shared-runtime/`: dependency-light safe-archive, configuration-apply, and provider-quota contracts shared by web/Electron.
  - `cursor-sdk-runtime/`: shared Cursor SDK execution/auth helper package used by web/Electron.
  - `orchestration-runtime/`: dependency-free DevRyan-managed task contracts and scheduler policy shared by web/Electron.
...
## Flow
1. Runtime package starts its host process (Electron main or Tauri main).
