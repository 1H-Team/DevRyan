...
- `lib/session-execution.js` gives every confined worker `OPENCODE_DISABLE_LSP_DOWNLOAD=true`, so download-backed language servers are not rebuilt in each scratch cache (`DEVRYAN_WORKER_LSP_DOWNLOAD=1`).
