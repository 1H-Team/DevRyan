...
- `lib/free-zen-model-catalog.js` — served-model plus zero-cost catalog intersection, five-minute cache, stale snapshot, and single-flight refresh shared by web/Electron.
- `lib/free-zen-generation.js` — sequential per-model free-Zen rotation plus shared PR title/body JSON normalization.
- `testing/assistant-image-fixtures.js` — golden parser fixtures shared by UI and web security tests.

## Where to change things

- Change cross-host ZIP security rules only in `lib/safe-archive.js`; keep web as thin adapters.
