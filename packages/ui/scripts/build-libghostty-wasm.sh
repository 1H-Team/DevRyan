...
CACHE_DIR="${DEVRYAN_GHOSTTY_CACHE:-${UI_DIR}/../../.cache/devryan-ghostty}"
...
ensure_zig() {
  ...
  local zig_archive="${CACHE_DIR}/zig-${host_arch}-${host_os}-${GHOSTTY_ZIG_VERSION}.tar.xz"
  local expected
  case "${host_arch}-${host_os}" in
    aarch64-macos) expected=3cc2bab367e185cdfb27501c4b30b1b0653c28d9f73df8dc91488e66ece5fa6b ;;
    x86_64-macos) expected=375b6909fc1495d16fc2c7db9538f707456bfc3373b14ee83fdd3e22b3d43f7f ;;
    aarch64-linux) expected=958ed7d1e00d0ea76590d27666efbf7a932281b3d7ba0c6b01b0ff26498f667f ;;
    x86_64-linux) expected=02aa270f183da276e5b5920b1dac44a63f1a49e55050ebde3aecc9eb82f93239 ;;
    *) die "unverified Zig platform" ;;
  esac
  curl -fsSL -o "${zig_archive}" \
    "https://ziglang.org/download/${GHOSTTY_ZIG_VERSION}/zig-${host_arch}-${host_os}-${GHOSTTY_ZIG_VERSION}.tar.xz"
  require_cmd shasum
  [[ "$(shasum -a 256 "${zig_archive}" | cut -d ' ' -f 1)" == "${expected}" ]] || die "Zig archive checksum mismatch"
  tar -xJ --strip-components=1 -C "${zig_dir}" -f "${zig_archive}"
}
...
ensure_macos_sdk_shim() {
  ...
  if grep -q "arm64-macos" "${sdk_path}/usr/lib/system/libsystem_c.tbd" 2>/dev/null; then
    return 0
  fi
...
ensure_zig
[[ "$("${GHOSTTY_ZIG}" version)" == "0.15.2" ]] || die "Zig 0.15.2 is required"
ensure_macos_sdk_shim
ensure_ghostty_source

mkdir -p "${CACHE_DIR}/builds"
build_root="$(mktemp -d "${CACHE_DIR}/builds/build.XXXXXX")"
