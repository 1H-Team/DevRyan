...
import { canvasFontFamilies, isMonospaceFamily } from './fonts';
import {
  GhosttyTerminalCore,
...
// Bundled with the app: prompt symbols must not depend on a CDN fetch, and the
// cell grid must be measured with the faces that will actually render.
...
// Compatibility mouse events follow a touch within this window; they must not
// summon the soft keyboard when the host owns touch gestures.
const TOUCH_MOUSE_COMPAT_WINDOW_MS = 1000;
...
function uncheckedTerminalFontFamily(family?: string): string {
  const custom = family === undefined ? null : canvasFontFamilies(family);
  return custom === null ? DEFAULT_TERMINAL_FONT_FAMILY : `${custom}, ${DEFAULT_TERMINAL_FONT_FAMILY}`;
}
...
export async function loadTerminalFontFamily(
  ...
  size: number,
  environment?: {
    readonly load: (font: string, text: string) => Promise<void>;
  ...
  const load =
    environment?.load ??
    ((font: string, text: string) => document.fonts.load(font, text).then(() => undefined));
  try {
    await Promise.all(
...
function isTerminalAltGraphText(
...
export interface TerminalMouseDataResolution {
  readonly send: boolean;
  readonly nextMotionData: string;
}

interface TerminalMouseTrackingState {
  readonly tracking: boolean;
  readonly motionData: string;
}

export interface TerminalWheelRows {
...
export interface GhosttyTerminalSurfaceOptions {
  ...
  /** Accessible names for the hidden input and the scrollbar, already localized by the host. */
  readonly labels: { readonly input: string; readonly scrollbar: string; readonly output?: string };
  /**
   * When false the surface ignores touch pointers (and the compatibility mouse
   * events that follow them) so the host can run its own scroll and
   * long-press selection gestures through `scrollLines`, `selectWordAt` and
   * `extendSelectionTo`. Defaults to true.
   */
  readonly handleTouchPointer?: boolean;
  readonly onData: (data: string) => void;
  readonly onResize: (cols: number, rows: number) => void;
  readonly onSelectionChange?: () => void;
  readonly onScroll?: (distanceFromBottom: number) => void;
  /** Return false to keep a key press from reaching the terminal (host shortcuts). */
...
export class GhosttyTerminalSurface {
  private readonly accessibleOutput = document.createElement('pre');
  ...
  private inputLeft = -1;
  private inputTop = -1;
  private lastTouchPointerAt = Number.NEGATIVE_INFINITY;
  ...
  private constructor(
    ...
    this.accessibleOutput.className = 'oc-terminal-accessible-output';
    this.accessibleOutput.setAttribute('role', 'region');
    this.accessibleOutput.setAttribute('aria-label', options.labels.output ?? 'Terminal output');
    this.accessibleOutput.setAttribute('aria-live', 'off');
    mount.append(this.accessibleOutput);
  ...
  static async create(
    ...
    canvas.className = 'oc-terminal-canvas';
    canvas.setAttribute('aria-hidden', 'true');

    const input = document.createElement('textarea');
    input.className = 'oc-terminal-input';
    input.setAttribute('aria-label', options.labels.input);
    ...
    scrollbar.className = 'oc-terminal-scrollbar';
    scrollbar.setAttribute('role', 'scrollbar');
    scrollbar.setAttribute('aria-label', options.labels.scrollbar);
    scrollbar.setAttribute('aria-orientation', 'vertical');
    scrollbar.tabIndex = -1;
    scrollbar.hidden = true;
    const scrollbarThumb = document.createElement('div');
    scrollbarThumb.className = 'oc-terminal-scrollbar-thumb';
    scrollbar.append(scrollbarThumb);
    mount.replaceChildren(canvas, input, scrollbar);
    // Size the backing store before the browser paints this canvas for the
    // first time. The font loads below yield to the compositor, and a canvas
    // whose intrinsic size later changes while its CSS box stays put (the box
    // is 100% of the mount) keeps showing the first, stretched image in
    // Gecko until something else invalidates layout — a brand-new tab was
    // blurry until a reload or a panel resize.
    const initialRatio = window.devicePixelRatio || 1;
    if (mount.clientWidth > 0 && mount.clientHeight > 0) {
      canvas.width = Math.max(1, Math.round(mount.clientWidth * initialRatio));
      canvas.height = Math.max(1, Math.round(mount.clientHeight * initialRatio));
    }

    // willReadFrequently pins the canvas to the software rasterizer. Gecko
    // otherwise decides per canvas whether to accelerate it, and its GPU text
    // path on macOS skips CoreText smoothing: a terminal created after page
    ...
    const metrics = measureGhosttyCell(context, fontSize, fontFamily, options.lineHeight);
  ...
  write(data: string, replay = false): void {
    if (this.disposed) return;
    if (replay) this.core.writeReplay(data);
    else this.core.write(data);
  ...
  /**
   * Replaces the screen with replayed history. History was laid out by the
   * shell for the PTY size it had at the time; when that differs from the
   * fitted grid the replay runs at the drawn size and Ghostty reflows to the
   * fitted grid afterwards, so lines wrap where the shell wrapped them.
   */
  resetAndWrite(data: string, drawnSize?: { readonly cols: number; readonly rows: number }): void {
    if (this.disposed) return;
    this.lastMouseMotionData = '';
    const replayAtDrawnSize =
      drawnSize !== undefined && (drawnSize.cols !== this.cols || drawnSize.rows !== this.rows);
    if (replayAtDrawnSize) {
      this.core.resize(drawnSize.cols, drawnSize.rows, this.metrics.width, this.metrics.height);
    }
    try {
  ...
  private applyFontMetrics(): void {
    this.metrics = measureGhosttyCell(this.context, this.fontSize, this.fontFamily, this.options.lineHeight);
    this.core.resize(this.cols, this.rows, this.metrics.width, this.metrics.height);
  ...
  private readonly onFontsLoaded = () => {
    ...
    const metrics = measureGhosttyCell(this.context, this.fontSize, this.fontFamily, this.options.lineHeight);
    if (
  ...
  /**
   * Repaints every row on the next frame. Hosts call this once a container
   * transition (panel open, resize handle release) has settled: the canvas
   * itself did not change, but a compositor that cached it mid-transition
   * only drops that cache when the canvas contents update.
   */
  refresh(): void {
    if (this.disposed) return;
    this.forceFullRender = true;
  ...
  /** Scrolls the viewport by whole rows; positive moves toward the bottom. */
  scrollLines(deltaRows: number): void {
    if (this.disposed) return;
    this.scrollViewport(deltaRows);
  }

  /** Starts a word selection at a client point (touch long-press). Returns whether a word was found. */
  ...
  /** Extends the current selection to a client point, keeping the selection mode's granularity. */
  extendSelectionTo(clientX: number, clientY: number): void {
    if (this.disposed || this.selectionAnchorScreen === null) return;
    const cell = this.cellAt(clientX, clientY);
    if (cell.x === this.selectionEnd?.x && cell.y === this.selectionEnd.y) return;
    this.extendSelectionToPoint(clientX, clientY);
  }
  ...
  private ignoresTouchPointer(event: PointerEvent): boolean {
    if (event.pointerType !== 'touch' || this.options.handleTouchPointer !== false) return false;
    this.lastTouchPointerAt = event.timeStamp;
    return true;
  }

  private readonly onPointerDown = (event: PointerEvent) => {
    if (this.ignoresTouchPointer(event)) return;
    this.focus();
  ...
  private readonly onPointerMove = (event: PointerEvent) => {
    ...
    const cell = this.cellAt(event.clientX, event.clientY);
    if (cell.x === this.selectionEnd?.x && cell.y === this.selectionEnd.y) return;
    this.extendSelectionToPoint(event.clientX, event.clientY);
  };

  private extendSelectionToPoint(clientX: number, clientY: number): void {
  ...
  private setSelectionAutoscroll(delta: number): void {
    ...
    this.selectionScrollTimer = window.setInterval(() => {
      ...
      if (pointer) this.extendSelectionToPoint(pointer.x, pointer.y);
    }, 80);
  }
  ...
  private readonly onMouseDown = (event: MouseEvent) => {
    ...
    if (event.timeStamp - this.lastTouchPointerAt < TOUCH_MOUSE_COMPAT_WINDOW_MS) return;
    this.focus();
  };
  ...
  readBuffer() { const buffer = this.core.readBuffer(); this.refresh(); return buffer; }
  getViewportY(): number { const state = this.core.scrollbarState(); return state ? state.total - state.len - state.offset : 0; }
  hasBracketedPaste(): boolean { return this.core.encodePaste('').startsWith('\x1b[200~'); }
  setCursorStyle(style: 'bar' | 'block' | 'underline'): void { this.core.write(`\x1b[${style === 'bar' ? 6 : style === 'underline' ? 4 : 2} q`); this.refresh(); }
  setCursorBlink(enabled: boolean): void { this.core.write(enabled ? '\x1b[?12h' : '\x1b[?12l'); this.refresh(); }

  private readScrollbarState(): GhosttyScrollbar | null {
    const state = this.core.scrollbarState();
    const previous = this.scrollbarState;
    this.scrollbarState = state;
    if (state && (!previous || state.offset !== previous.offset || state.total !== previous.total || state.len !== previous.len))
      this.options.onScroll?.(state.total - state.len - state.offset);
    return state;
  }
  ...
  private renderFrame(): void {
    ...
    const accessibleText = this.snapshot.rowData.map((row) => row.text).join('\n');
    if (this.accessibleOutput.textContent !== accessibleText) this.accessibleOutput.textContent = accessibleText;
  ...
  private cellAt(clientX: number, clientY: number): GhosttyGridPoint {
    const bounds = this.canvas.getBoundingClientRect();
    return {
      x: Math.max(
        0,
