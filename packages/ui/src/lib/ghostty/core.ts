...
export interface GhosttyTheme {
  ...
  /**
   * The 16 ANSI colors (normal then bright). Indices 16-255 keep the standard
   * xterm cube and gray ramp. Omitted, Ghostty's built-in palette applies.
   */
...
/** The xterm 256-color table with the first 16 entries replaced by the theme's ANSI colors. */
export function ghosttyPaletteBytes(ansi: readonly GhosttyColor[]): Uint8Array {
  const bytes = new Uint8Array(PALETTE_SIZE * 3);
  const put = (index: number, color: GhosttyColor) => {
  ...
  for (let index = 16; index < 232; index += 1) {
    ...
    put(index, {
      r: cubeLevels[Math.floor(value / 36) % 6] ?? 0,
      g: cubeLevels[Math.floor(value / 6) % 6] ?? 0,
      b: cubeLevels[value % 6] ?? 0,
    });
  }
  for (let index = 232; index < PALETTE_SIZE; index += 1) {
    const gray = 8 + (index - 232) * 10;
    put(index, { r: gray, g: gray, b: gray });
  }
  ansi.slice(0, 16).forEach((color, index) => put(index, color));
  return bytes;
}
...
export class GhosttyTerminalCore {
  ...
  /** Parse retained output without answering historical queries to the PTY. */
  ...
  setTheme(theme: GhosttyTheme): void {
    ...
    // Option 14 takes GhosttyColorRgb[256]; NULL restores the built-in table.
    // Per-index OSC 4 overrides a program set survive either way.
    if (theme.palette === undefined) {
      this.runtime.call('ghostty_terminal_set', this.terminal, 14, 0);
      return;
    }
    const palette = ghosttyPaletteBytes(theme.palette);
    const pointer = this.runtime.alloc(palette.length);
    this.runtime.bytes(pointer, palette.length).set(palette);
    this.runtime.call('ghostty_terminal_set', this.terminal, 14, pointer);
    this.runtime.free(pointer, palette.length);
  }
  ...
  readBuffer(): { rows: GhosttyRow[]; baseY: number; cursorX: number; cursorY: number } {
    const original = this.scrollbarState();
    const rows: GhosttyRow[] = [];
    try {
      this.scrollToBottom();
      const bottom = this.snapshot();
      const state = this.scrollbarState();
      const total = state?.total ?? bottom.rows;
      this.scroll(-(state?.offset ?? 0));
      for (let index = 0; index < total;) {
        const view = this.snapshot();
        const offset = this.scrollbarState()?.offset ?? 0;
        const start = Math.max(0, index - offset);
        for (let n = start; n < view.rowData.length && index < total; n++, index++) rows.push(view.rowData[n]!);
        if (index >= total) break;
        this.scroll(view.rows);
        if ((this.scrollbarState()?.offset ?? 0) === offset) break;
      }
      return { rows, baseY: Math.max(0, total - bottom.rows), cursorX: bottom.cursorX, cursorY: bottom.cursorY };
    } finally {
      this.scrollToBottom();
      if (original) this.scroll(original.offset - (this.scrollbarState()?.offset ?? 0));
    }
  }
