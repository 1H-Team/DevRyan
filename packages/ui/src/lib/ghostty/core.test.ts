...
import { loadFixtureRuntime } from './runtime.fixture';
...
describe('GhosttyTerminalCore', () => {
  ...
  async function createCore(onData: (data: string) => void = () => {}, palette?: GhosttyColor[]) {
    const core = await GhosttyTerminalCore.create(12, 3, 8, 16, {
      ...
      cursor: WHITE,
      palette,
    }, onData, await loadFixtureRuntime());
    cores.add(core);
    return core;
  }
...
test('complete buffer reads retain scrollback, soft wraps and the scrolled viewport', async () => {
  const core = await GhosttyTerminalCore.create(12, 3, 8, 16, { foreground: WHITE, background: BLACK, cursor: WHITE }, () => {}, await loadFixtureRuntime());
  try {
    core.write(Array.from({ length: 80 }, (_, i) => `line${i}`).join('\r\n'));
    core.scroll(-17);
    const before = core.scrollbarState();
    const buffer = core.readBuffer();
    expect(buffer.rows[0].text).toBe('line0');
    expect(buffer.rows[79].text).toBe('line79');
    expect(buffer.baseY + buffer.cursorY).toBe(79);
    expect(core.scrollbarState()).toEqual(before);
    core.resetAndWrite('123456789012abcdef');
    const wrapped = core.readBuffer();
    expect(wrapped.rows[1].isWrapContinuation).toBe(true);
