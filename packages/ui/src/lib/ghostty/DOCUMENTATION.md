...
The surface handles native copy/paste events and keyboard shortcuts, including bracketed paste and IME deduplication. The viewport retains Electron menu-copy integration and its existing touch gestures. Links pass through DevRyan's safe external-URL opener. The canvas is hidden from accessibility APIs; a labeled input and bounded, non-live text output expose terminal content without announcing every output frame. There is no additional desktop context menu in this adapter.
...
2. Run `bun run --cwd packages/ui build:ghostty-wasm`. It downloads Zig 0.15.2 into `.cache/devryan-ghostty`, clones Ghostty at the pin, builds `wasm32-freestanding`, replaces `vendor/ghostty-vt.wasm`, and prints the trampoline bytes for `runtime.ts` (they only change when the Zig source changes).
