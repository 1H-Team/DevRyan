...
## Flow
Views/hooks request git data, then transform results for tables and diff widgets. The Source-tab commit generator uses staged changes when present, otherwise all worktree changes, and makes one host request. Web/Electron and VS Code collect authoritative context and run the session-free utility model without client-side Git-request fan-out.
