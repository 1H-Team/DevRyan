...
const persistToLocalStorage = (settings: DesktopSettings) => {
  ...
  if (settings.homeDirectory) {
    ...
    // contextBridge property; assignment throws TypeError there. In the
    // plain web runtime the property is writable. Swallow the
...
const readCurrentModelPrefs = (): ModelPrefsSnapshot => {
...
const applyDesktopUiPreferences = (
  ...
  if (hasFavoriteModels || hasHiddenModels || hasFavoriteModelsUpdatedAt || hasHiddenModelsUpdatedAt) {
    ...
    const next = options?.modelPrefsBaseline
      ? resolveModelPrefsFromSettingsSnapshot({
          baseline: options.modelPrefsBaseline,
          current,
          incoming,
        })
      : incoming;

    const modelPrefsUpdate: Partial<ModelPrefsSnapshot> = {};
