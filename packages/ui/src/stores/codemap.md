...
- **Session plan-file pointer**: `useSessionPlanFileStore.ts` owns the narrow non-persisted `saving | saved | error` record for the latest plan revision in each session. Permanent session deletion clears the pointer but intentionally leaves the Markdown artifact intact; archive preserves both.
