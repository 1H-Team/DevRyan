...
import { RiAppleLine, RiRobot2Line } from '@remixicon/react';

import { BotChatView } from '@/components/bots/chat/BotChatView';
import { VSCODE_UNSUPPORTED_BOT_ID } from '@/components/bots/sidebar/BotSidebarSection';
import { isVSCodeRuntime } from '@/lib/desktop';
import { useI18n } from '@/lib/i18n';
import { botChannelSelectors, useBotChannelStore } from '@/stores/useBotChannelStore';
import { useBotsStore } from '@/stores/useBotsStore';

const UnsupportedBotsView: React.FC<{ vscode: boolean }> = ({ vscode }) => {
  const { t } = useI18n();
  return (
    <div className="flex h-full items-center justify-center px-6 text-center" data-bot-unsupported-host>
      <div className="max-w-md">
        {vscode ? (
          <RiAppleLine className="mx-auto h-9 w-9 text-muted-foreground" aria-hidden />
        ) : (
        ...
        <h1 className="mt-4 typography-ui-header font-semibold text-foreground">
          {vscode ? t('bots.vscode.required') : t('bots.runtime.unsupportedHost')}
        </h1>
        <p className="mt-2 typography-body text-muted-foreground">
          {vscode ? t('bots.vscode.description') : t('bots.runtime.unsupportedDescription')}
        </p>
      </div>
    </div>
  );
...
export const BotView: React.FC = () => {
  ...
  const vscode = isVSCodeRuntime()
    || selectedBotId === VSCODE_UNSUPPORTED_BOT_ID
    || capabilities?.owner === 'vscode';

  if (vscode) return <UnsupportedBotsView vscode />;
  if (capabilities?.state === 'unsupported_host') return <UnsupportedBotsView vscode={false} />;
  if (!selectedBotId) {
    return (
