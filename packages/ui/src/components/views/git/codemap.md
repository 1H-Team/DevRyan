...
`PullRequestSection.tsx` exclusively owns PR presentation. It lands on the all-state paginated repository-network browser, exposes a current-branch action for creating or managing that branch's PR, opens the same workflow when the current PR is selected, and renders non-current selections read-only. Terminal PRs remain linked while a persisted next-cycle draft is composed for the same long-lived branch, and low-frequency terminal discovery can replace them with a newly opened PR.
