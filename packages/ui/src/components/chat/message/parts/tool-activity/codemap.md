...
`classification.ts` routes legacy Context Mode (`ctx_*`) tool parts from stored sessions through the standard expandable tool disclosure so their error guidance and partial output remain inspectable in Live and Sorted modes.
