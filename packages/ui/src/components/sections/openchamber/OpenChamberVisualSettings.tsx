...
export type VisibleSetting = 'theme' | 'pwaInstallName' | 'pwaOrientation' | 'mobileKeyboardMode' | 'timeFormat' | 'weekStart' | 'fontSize' | 'chatWidth' | 'codeFont' | 'terminalFontSize' | 'spacing' | 'inputBarOffset' | 'mermaidRendering' | 'userMessageRendering' | 'collapsibleUserMessages' | 'stickyUserHeader' | 'splitAssistantMessageActions' | 'diffLayout' | 'mobileStatusBar' | 'dotfiles' | 'reasoning' | 'showToolFileIcons' | 'expandedTools' | 'queueMode' | 'terminalQuickKeys';
...
export const OpenChamberVisualSettings: React.FC<OpenChamberVisualSettingsProps> = ({ visibleSettings }) => {
    ...
    const shouldShow = (setting: VisibleSetting): boolean => {
        if (!isVisualSettingAllowedByPolicy(setting, principal)) {
            return false;
        }
    ...
    const hasLayoutSettings = shouldShow('fontSize') || shouldShow('chatWidth') || shouldShow('codeFont') || shouldShow('terminalFontSize') || shouldShow('spacing') || shouldShow('inputBarOffset');
        ...
        <div className="flex flex-col gap-8">
                ...
                {hasLayoutSettings && (
                    <div className="order-5 space-y-3">
                        <section className="p-2 space-y-0.5">
                            ...
                            {shouldShow('chatWidth') && !isMobile && (
                                <div className="flex items-center gap-8 py-1">
                                    <div className="flex min-w-0 flex-col w-56 shrink-0">
                                        <span className="typography-ui-label text-foreground">{t('settings.openchamber.visual.field.chatWidth')}</span>
                                    </div>
                                    <div className="flex items-center gap-2 w-fit">
                                        <Slider
                                            value={chatWidth}
                                            ...
                                            label={t('settings.openchamber.visual.field.chatWidthAria')}
                                        />
                                        <Button size="sm"
                                            type="button"
                                            variant="ghost"
                                            onClick={() => setChatWidth(DEFAULT_CHAT_WIDTH)}
                                            disabled={chatWidth === DEFAULT_CHAT_WIDTH}
                                            className="h-7 w-7 px-0 text-muted-foreground hover:text-foreground"
                                            aria-label={t('settings.openchamber.visual.actions.resetChatWidthAria')}
                                            title={t('settings.common.actions.reset')}
                                        >
                                            <RiRestartLine className="h-3.5 w-3.5" />
                                        </Button>
                ...
                {hasBehaviorSettings && (
                    <div className="order-2 space-y-3">
                            ...
                            {(shouldShow('userMessageRendering') || shouldShow('mermaidRendering') || (shouldShow('diffLayout'))) && (
                                        ...
                                        >
                                            ...
                                            <div className="flex min-w-0 items-center gap-1.5">
                                                ...
                                                <Tooltip>
                                                    ...
                                                    <TooltipContent sideOffset={8} className="max-w-xs">
                                                        {t('settings.openchamber.visual.field.showReasoningTracesTooltip')}
                                                    </TooltipContent>
                                                </Tooltip>
                                            </div>
                                        </div>
