...
export const Sidebar: React.FC<SidebarProps> = ({ isOpen, isMobile, children, className }) => {
    ...
    const applyLiveWidth = React.useCallback((nextWidth: number) => {
        ...
        if (content) {
            content.style.setProperty('--oc-left-sidebar-content-width', `${nextWidth}px`);
        }
    }, []);
    ...
    // Inner content keeps a stable width while the outer aside animates, so the
    // content does not reflow mid-collapse — it slides/fades and is clipped.
    const contentWidth = isOpen
        ? Math.min(SIDEBAR_MAX_WIDTH, Math.max(SIDEBAR_MIN_WIDTH, sidebarWidth || SIDEBAR_CONTENT_WIDTH))
        : Math.min(SIDEBAR_MAX_WIDTH, Math.max(SIDEBAR_MIN_WIDTH, sidebarWidth || SIDEBAR_CONTENT_WIDTH));
    ...
    return (
        <aside
            ...
            className={cn(
                ...
                'bg-sidebar',
                isResizing || prefersReducedMotion ? 'transition-none' : 'transition-[width,min-width,max-width] duration-300 ease-in-out',
                !isOpen && 'border-r-0',
                className,
            )}
            style={{
        ...
        >
            ...
            <div
                ...
                className={cn(
                    ...
                    !prefersReducedMotion && !isResizing && 'transition-[transform,opacity] duration-200 ease-out',
                )}
                style={{
                    width: 'var(--oc-left-sidebar-content-width)',
                    minWidth: 'var(--oc-left-sidebar-content-width)',
                    transform: !isOpen && !prefersReducedMotion ? 'translateX(-12px)' : 'translateX(0)',
                    overflowX: 'hidden',
                    ['--oc-left-sidebar-content-width' as string]: `${contentWidth}px`,
                }}
                aria-hidden={!isOpen}
            >
