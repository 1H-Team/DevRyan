...
Plan mode reaches the model through the synthetic plan-mode preface. The prompt transport resolves only provider/agent tool overrides (`resolveProviderPromptTools`) before every primary fetch attempt, including queued sends and retries; Plan Mode adds no tool overrides of its own.
