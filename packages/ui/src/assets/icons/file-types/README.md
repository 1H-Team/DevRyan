...
- `packages/ui/src/lib/fileTypeSprite.ts` loads `sprite.svg` as a lazy raw-text chunk and mounts it in one hidden document root.
- `packages/ui/src/components/icons/FileTypeIcon.tsx` renders `<use href="#icon-id" />` after that shared symbol root is available.
