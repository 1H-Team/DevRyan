...
## Responsibility
Workspace package for the shared React UI runtime used by web and Electron shells. It owns app composition, feature components, client-side state, sync/event handling, and runtime API bridging.
...
- **Authoritative plan files**: `src/lib/plans/sessionPlanPersistence.ts` delegates create-once storage to the runtime `SessionPlansAPI`, deduplicates lifecycle/card saves, and updates `src/stores/useSessionPlanFileStore.ts`. Web/Electron use the scoped server plan-revision route.
...
- **Cross-package contract**: shared types and runtime capability gates (desktop/web) keep one UI codepath across shells.
