...
- Tool-call normalization preserves explicit SDK terminal status. Partial task/tool output survives a later error or cancellation, while provider failure reasons remain attached to the same canonical tool part. `metadata.syntheticWorkspacePatch` is runtime-reserved: provider metadata is stripped of that key while unrelated fields survive, and only the derived workspace patch producer sets it after building the canonical card. Evidence consumers therefore combine the marker with the exact runtime patch-card identity and payload shape instead of trusting provider metadata alone.
