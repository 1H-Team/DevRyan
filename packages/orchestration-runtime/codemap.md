...
- `provider-prompt-tools.js`: shared provider and managed read-only tool profiles. Managed read-only children retain a wildcard-deny inspection policy.
...
- `provider-retry-policy.js`: shared provider policy classifiers for definite usage/quota exhaustion (including xAI spending-limit blocks), verified provider prompt rejection, and normalized request/header/stream-idle/connection transport failures, with auth/model/certificate/abort precedence.
