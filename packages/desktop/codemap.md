...
- **Compatibility regression suite**: `bun run test` runs `cargo test --locked`; health probes use loopback fixtures and accept only a valid DevRyan payload with `openCodeRunning: true` as ready. Rust/Cargo/Tauri changes select this suite in quick and affected validation.
