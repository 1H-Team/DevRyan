...
| Private gateway relay | Reasoning and computer containers hold no route off their internal networks. Authenticated private-gateway calls go to the bot-egress service at its in-network address, which relays them to the host loopback address Electron published on the egress control channel. | A container can neither supply nor discover the host address, and the relay forwards only the four gateway routes with a header set it builds itself, so the gateway's Host and bearer-capability checks still decide every call. The relay fails closed until the control channel publishes an address. |
| Runtime inbound path | Neither container publishes a host port. The supervisor shares both internal networks and hands the host one revocable scoped path per container. | Docker will not publish a port for an internal-only container, and a bridge added to get one back would also hand the container a route to the host and the public internet on Docker Desktop, masquerade or not. The `devryan-bots-host-control` bridge now carries only first-party services that publish loopback ports, and the Electron manager removes any Bot container still attached to it. |
