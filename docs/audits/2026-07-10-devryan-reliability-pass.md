...
| P1 | New managed-runtime workspace dependencies are absent from the resolved lock | fail | Final host-ABI restoration added the missing UI, and web `@openchamber/orchestration-runtime` workspace edges to `bun.lock` |
...
| Full production build | 42.84 s / 2,152,890,368-byte peak RSS | 45.67 s / 2,466,709,504-byte peak RSS | Same command but different checkout/cache state and a larger shared UI/VS Code surface; existing bundle CommonJS `import.meta` warnings remain, so no optimization claim is made |
...
- Web/Electron managed OpenCode no longer use `--pure` or suppress
...
- host one scheduler per web/Electron owner, connect bundled tools
...
- Current discovery still reports Anthropic, Google, and OpenCode Go as
  ...
  claim.
- Web/Electron share the managed plugin, orchestration executor,
...
- `bun run build`: pass for web and Electron with only the existing
