...
DevRyan is a Bun/Node monorepo that provides web and desktop UI runtimes for interacting with an OpenCode server. The shared React UI lives in `packages/ui`; `packages/web` owns the Express server, browser bootstrap, CLI, and the optional Supabase-backed shared-host control plane; `packages/shared-runtime` owns dependency-light configuration-apply, quota-normalization, assistant-image syntax, and safe-archive rules shared by web/Electron; `packages/bots-runtime` owns dependency-free Production Bots JSON contracts plus the scoped OpenCode image/plugin; `packages/bot-supervisor` owns the fixed-verb Docker lifecycle boundary without socket access; `packages/bot-engine-proxy` is the sole Docker-socket process; `packages/bot-egress` owns purpose-separated model, AG-UI endpoint, and browser network policy; `packages/bot-computer` owns the persistent reviewed-command Chromium service; `packages/bot-indexer` owns the disposable Docker-local FTS/vector retrieval projection; `packages/cursor-sdk-runtime` owns shared Cursor SDK execution/auth helpers; `packages/harness-runtime` owns durable harness operations, diagnostics, lifecycle, evidence, and cross-process persistence primitives; `packages/electron` is the primary desktop shell and optional launchd runtime-service executable; `packages/desktop` is the legacy Tauri shell.
...
- `scripts/qa/run.mjs`: isolated web/Electron QA; debugging and live-provider usage in `docs/QA.md`.
...
| `packages/shared-runtime/` | Dependency-light safe-archive, configuration-apply, and provider-quota contracts shared by web/Electron. | [packages/shared-runtime/codemap.md](packages/shared-runtime/codemap.md) |
...
| `packages/cursor-sdk-runtime/` | Shared Cursor SDK execution runtime used by web/Electron; quota credentials remain deliberately separate in each surface's quota module. | [packages/cursor-sdk-runtime/codemap.md](packages/cursor-sdk-runtime/codemap.md) |
| `packages/orchestration-runtime/` | Dependency-free DevRyan-managed task contract, dispatch-barrier, and scheduler policy shared by web/Electron owners. | [packages/orchestration-runtime/codemap.md](packages/orchestration-runtime/codemap.md) |
...
- **Session creation latency/cancellation** → `packages/ui/src/sync/session-creation.ts`, `session-actions.ts`, `session-ui-store.ts`, and `packages/web/server/lib/opencode/session-creation.js`. Packaged-renderer fixture: `tests/visual-session-creation/codemap.md`; results: `docs/SESSION_CREATION_VERIFICATION.md`.
...
- **Primary provider stall/recovery policy** → `packages/harness-runtime/lib/provider-recovery*.js`, web harness host adapters; see `docs/PROVIDER_RECOVERY.md`. Shared UI: `HostPrimaryRecovery.tsx`; isolated fixtures: `tests/visual-provider-recovery/` and `tests/provider-recovery/`.
...
- **Cross-host ZIP installation safety, batched configuration apply state, provider quota normalization, or assistant-image syntax parity** → `packages/shared-runtime/codemap.md` and `packages/shared-runtime/DOCUMENTATION.md`; host adapters remain in web.
...
1. A host runtime starts or connects to OpenCode: web CLI/server, Electron main process, or legacy Tauri sidecar.
2. The host exposes runtime APIs over HTTP, WebSocket/SSE, or desktop IPC.
...
5. Live OpenCode events flow through server event streams into UI sync stores; managed-host events are ownership-filtered and projected into the content-free actor audit feed.
...
8. Packaging scripts build the shared UI/server outputs into Electron, legacy Tauri, or standalone web deployments.
...
Discovered session/worktree ownership is retained in bounded principal-scoped navigation history across reloads; it never restores a live branch or authorizes execution.
