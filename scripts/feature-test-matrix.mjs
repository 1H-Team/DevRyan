export const FEATURE_TEST_MATRIX = Object.freeze([
  {
    id: 'context-mode-worker-liveness',
    sourcePaths: ['packages/web/server/lib/opencode/context-mode-worker-pool.js',
      'packages/web/server/lib/opencode/context-mode-worker.js', 'packages/web/server/lib/opencode/context-mode-native-hotfix.js',
      'packages/web/server/lib/opencode/context-mode-worker-state.js', 'packages/web/server/lib/opencode/context-mode-worker-storage.js',
      'packages/web/server/lib/opencode/context-mode-worker-process.js',
      'packages/web/server/lib/opencode/context-mode-storage-hotfix.js',
      'packages/web/server/lib/opencode/context-mode-diagnostics.js', 'scripts/qa/context-mode.mjs',
      'packages/ui/src/components/chat/message/parts/WorkingPlaceholder.tsx',
      'packages/ui/src/components/chat/message/parts/workingPlaceholderStatus.ts',
      'packages/ui/src/components/chat/message/parts/tool-activity/classification.ts'],
    testPaths: ['packages/web/server/lib/opencode/context-mode-worker-liveness.test.js',
      'packages/web/server/lib/opencode/context-mode-worker-pool.test.js', 'packages/web/server/lib/opencode/context-mode-hotfix.test.js',
      'packages/web/server/lib/opencode/context-mode-worker-state.test.js',
      'packages/web/server/lib/opencode/context-mode-worker-process.test.js',
      'packages/web/server/lib/opencode/context-mode-diagnostics.test.js', 'scripts/qa/context-mode.test.mjs',
      'packages/ui/src/components/chat/message/parts/workingPlaceholderStatus.test.ts',
      'packages/ui/src/components/chat/message/parts/toolRenderUtils.test.ts'],
  },
  {
    id: 'isolated-web-electron-qa',
    sourcePaths: ['scripts/qa/run.mjs', 'scripts/qa/cdp.mjs', 'scripts/qa/process.mjs', 'scripts/perf/loopback-opencode-fixture.mjs'],
    testPaths: ['scripts/qa/run.test.mjs', 'scripts/qa/cdp.test.mjs', 'scripts/qa/process.test.mjs', 'scripts/perf/loopback-opencode-fixture.test.mjs'],
  },
  {
    id: 'ui-session-chat-sync',
    sourcePaths: ['packages/ui/src/sync/sync-context.tsx', 'packages/ui/src/sync/session-ui-store.ts'],
    testPaths: ['packages/ui/src/sync/sync-context.plan-lifecycle.test.ts', 'packages/ui/src/sync/session-ui-store.send.test.ts'],
  },
  {
    id: 'web-server-and-runtime-apis',
    sourcePaths: ['packages/web/server/index.js', 'packages/web/src/api/index.ts'],
    testPaths: ['packages/web/server/opencode-proxy.test.js', 'packages/web/src/api/index.test.ts'],
  },
  {
    id: 'multi-user-authentication',
    sourcePaths: [
      'packages/web/server/lib/multi-user/runtime.js',
      'packages/web/server/lib/multi-user/auth-compat.js',
      'packages/ui/src/components/auth/SessionAuthGate.tsx',
      'packages/ui/src/components/auth/sessionAuthState.ts',
    ],
    testPaths: [
      'packages/web/server/lib/multi-user/runtime.auth.test.js',
      'packages/web/server/lib/multi-user/auth-compat.test.js',
      'packages/ui/src/components/auth/sessionAuthState.test.ts',
    ],
  },
  {
    id: 'electron-native-shell',
    sourcePaths: ['packages/electron/main.mjs', 'packages/electron/preload.mjs'],
    testPaths: ['packages/electron/tests/origin-policy.test.mjs', 'packages/electron/tests/quit-cleanup.test.mjs'],
  },
  {
    id: 'legacy-tauri-compatibility',
    sourcePaths: ['packages/desktop/src-tauri/src/main.rs', 'packages/desktop/src-tauri/src/remote_ssh.rs'],
    testPaths: ['packages/desktop/src-tauri/src/main.rs', 'packages/desktop/src-tauri/src/remote_ssh.rs'],
  },
  {
    id: 'cursor-sdk-runtime',
    sourcePaths: ['packages/cursor-sdk-runtime/index.js', 'packages/cursor-sdk-runtime/cursor-question-runtime.js'],
    testPaths: ['packages/cursor-sdk-runtime/tool-call-normalize.test.js', 'packages/cursor-sdk-runtime/cursor-question-runtime.test.js'],
  },
  {
    id: 'managed-orchestration',
    sourcePaths: ['packages/orchestration-runtime/scheduler.js', 'packages/web/server/lib/orchestration/runtime.js'],
    testPaths: ['packages/orchestration-runtime/scheduler.lifecycle.test.js', 'packages/web/server/lib/orchestration/runtime.test.js', '.opencode/plugins/orchestrator-autoresume.test.mjs'],
  },
  {
    id: 'production-bots-runtime-contract',
    sourcePaths: [
      'packages/bots-runtime/contract.js',
      'packages/bots-runtime/strict-json.js',
      'packages/bots-runtime/lifecycle.js',
      'packages/bots-runtime/policy.js',
      'packages/bots-runtime/run-state.js',
      'packages/bots-runtime/routines.js',
    ],
    testPaths: [
      'packages/bots-runtime/contract.test.js',
      'packages/bots-runtime/strict-json.test.js',
      'packages/bots-runtime/lifecycle.test.js',
      'packages/bots-runtime/policy.test.js',
      'packages/bots-runtime/run-state.test.js',
      'packages/bots-runtime/routines.test.js',
    ],
  },
  {
    id: 'production-bots-agent-adapters-spec-and-policy-v2',
    sourcePaths: [
      'packages/web/server/lib/bots/reasoning-adapter.js',
      'packages/web/server/lib/bots/opencode-reasoning-adapter.js',
      'packages/web/server/lib/bots/ag-ui-reasoning-adapter.js',
      'packages/web/server/lib/bots/run-dispatcher.js',
      'packages/web/server/lib/bots/agent-connections.js',
      'packages/web/server/lib/bots/bot-spec.js',
      'packages/web/server/lib/bots/config-compiler.js',
      'packages/web/server/lib/bots/policy-engine.js',
      'packages/web/server/lib/bots/action-gateway.js',
    ],
    testPaths: [
      'packages/web/server/lib/bots/reasoning-adapter.conformance.test.js',
      'packages/web/server/lib/bots/ag-ui-reasoning-adapter.test.js',
      'packages/web/server/lib/bots/run-dispatcher.test.js',
      'packages/web/server/lib/bots/bot-spec.test.js',
      'packages/web/server/lib/bots/config-compiler.test.js',
      'packages/web/server/lib/bots/policy-engine.test.js',
      'packages/web/server/lib/bots/action-gateway.test.js',
      'supabase/tests/agent_agnostic_bots_program.test.sql',
    ],
  },
  {
    id: 'production-bots-memory-control-and-image-repair',
    sourcePaths: [
      'packages/web/server/lib/bots/memory-runtime.js',
      'packages/web/server/lib/bots/action-gateway.js',
      'packages/web/server/lib/bots/browser-service.js',
      'packages/web/server/lib/bots/opencode-provider.js',
      'packages/bots-runtime/opencode/devryan-bot-tools.mjs',
      'supabase/migrations/20260830150000_bot_waiting_control.sql',
    ],
    testPaths: [
      'packages/web/server/lib/bots/memory-runtime.test.js',
      'packages/web/server/lib/bots/action-gateway.test.js',
      'packages/web/server/lib/bots/browser-service.test.js',
      'packages/web/server/lib/bots/opencode-provider.test.js',
      'packages/bots-runtime/opencode/devryan-bot-tools.test.mjs',
      'packages/web/server/lib/multi-user/bot-waiting-control-migration.test.js',
      'supabase/tests/bot_waiting_control.test.sql',
    ],
  },
  {
    id: 'production-bots-runtime-service',
    sourcePaths: [
      'packages/electron/runtime-service.mjs',
      'packages/electron/runtime-service-native.mjs',
      'packages/electron/runtime-service-registration.mjs',
      'packages/electron/runtime-service-startup.mjs',
      'packages/electron/native/runtime-service-control/RuntimeServiceControl.mm',
      'packages/electron/scripts/build-runtime-service-control.mjs',
      'packages/electron/scripts/verify-runtime-service-package.mjs',
      'packages/electron/desktop-host-broker.mjs',
      'packages/web/server/lib/runtime-service/routes.js',
      'packages/ui/src/components/sections/bots/BotRuntimeServicePanel.tsx',
      'packages/ui/src/components/sections/bots/botRuntimeServicePresentation.ts',
      'packages/ui/src/lib/botsDesktopApi.ts',
    ],
    testPaths: [
      'packages/electron/tests/runtime-service.test.mjs',
      'packages/electron/tests/runtime-service-native.test.mjs',
      'packages/electron/tests/runtime-service-registration.test.mjs',
      'packages/electron/tests/runtime-service-startup.test.mjs',
      'packages/electron/tests/desktop-host-broker.test.mjs',
      'packages/electron/tests/runtime-service-source.test.mjs',
      'packages/web/server/lib/runtime-service/routes.test.js',
      'packages/ui/src/components/sections/bots/BotRuntimeServicePanel.test.tsx',
      'packages/ui/src/lib/botsDesktopApi.test.ts',
    ],
  },
  {
    id: 'production-bots-activity-and-visual-fixture',
    sourcePaths: [
      'packages/ui/src/components/bots/chat/BotMessageRow.tsx',
      'packages/ui/src/components/bots/operations/BotOperationsRail.tsx',
      'packages/ui/src/stores/useBotOperationsNavigationStore.ts',
      'tests/visual-production-bots/src/main.tsx',
      'scripts/capture-production-bots-visuals.mjs',
      'scripts/package-production-bots-visual-shell.mjs',
    ],
    testPaths: [
      'packages/ui/src/components/bots/operations/BotOperationsRail.test.tsx',
      'tests/visual-production-bots/matrix.test.mjs',
      'scripts/capture-production-bots-visuals.test.mjs',
      'scripts/package-production-bots-visual-shell.test.mjs',
    ],
  },
  {
    id: 'production-bots-container-confinement',
    sourcePaths: [
      'packages/bot-supervisor/src/docker.js',
      'packages/bot-supervisor/src/server.js',
      'packages/bot-supervisor/src/engine-proxy-client.js',
      'packages/bot-engine-proxy/src/server.js',
      'packages/bot-egress/src/connect-policy.js',
      'packages/bot-egress/src/server.js',
      'packages/bot-egress/src/token.js',
      'packages/bots-runtime/opencode/devryan-bot-tools.mjs',
      'packages/bot-computer/src/browser.js',
      'packages/bot-computer/src/browser-diagnostics.js',
      'packages/bot-computer/src/display.js',
      'packages/bot-computer/src/server.js',
      'packages/bot-computer/src/egress-proxy.js',
      'packages/bot-computer/src/managed-policy.js',
    ],
    testPaths: [
      'packages/bot-supervisor/src/docker.test.js',
      'packages/bot-supervisor/src/server.test.js',
      'packages/bot-engine-proxy/src/server.test.js',
      'packages/bot-egress/src/connect-policy.test.js',
      'packages/bot-egress/src/server.test.js',
      'packages/bot-egress/src/token.test.js',
      'packages/bots-runtime/opencode/devryan-bot-tools.test.mjs',
      'packages/bot-computer/src/browser.test.js',
      'packages/bot-computer/src/browser-diagnostics.test.js',
      'packages/bot-computer/src/display.test.js',
      'packages/bot-computer/src/server.test.js',
      'packages/bot-computer/src/egress-proxy.test.js',
      'packages/bot-computer/src/chromium-policy.test.js',
    ],
  },
  {
    id: 'production-bots-local-retrieval',
    sourcePaths: [
      'packages/bot-indexer/src/chunker.js',
      'packages/bot-indexer/src/embeddings.js',
      'packages/bot-indexer/src/index-store.js',
      'packages/bot-indexer/src/search.js',
      'packages/bot-indexer/src/server.js',
    ],
    testPaths: [
      'packages/bot-indexer/src/chunker.test.js',
      'packages/bot-indexer/src/embeddings.test.js',
      'packages/bot-indexer/src/index-store.test.js',
      'packages/bot-indexer/src/search.test.js',
      'packages/bot-indexer/src/server.test.js',
    ],
  },
  {
    id: 'diagnostics-evidence-worktrees',
    sourcePaths: ['packages/harness-runtime/lib/journal.js', 'packages/harness-runtime/lib/evidence-runtime.js', 'packages/harness-runtime/lib/worktree-bootstrap.js'],
    testPaths: ['packages/harness-runtime/lib/journal.test.js', 'packages/harness-runtime/lib/evidence-runtime.test.js', 'packages/harness-runtime/lib/worktree-bootstrap.test.js'],
  },
  {
    id: 'providers-and-quota',
    sourcePaths: ['packages/web/server/lib/quota/providers/index.js', 'packages/ui/src/stores/useQuotaStore.ts'],
    testPaths: ['packages/web/server/lib/quota/providers/index.test.js', 'packages/ui/src/stores/useQuotaStore.test.ts'],
  },
  {
    id: 'git-and-github',
    sourcePaths: ['packages/web/server/lib/git/service.js', 'packages/web/server/lib/git/commit-message.js', 'packages/web/server/lib/github/routes.js'],
    testPaths: [
      'packages/web/server/lib/git/service.test.js',
      'packages/web/server/lib/git/commit-message.test.js',
      'packages/web/server/lib/git/routes.commit-message.test.js',
      'packages/ui/src/lib/gitApi.generation.test.ts',
      'packages/ui/src/lib/github/sourceRepo.test.ts',
    ],
  },
  {
    id: 'terminal-runtime',
    sourcePaths: ['packages/web/server/lib/terminal/runtime.js', 'packages/ui/src/lib/terminalApi.ts'],
    testPaths: ['packages/web/server/lib/terminal/runtime.test.js', 'packages/ui/src/lib/terminalApi.test.ts'],
  },
  {
    id: 'skills-plugins-and-mcp',
    sourcePaths: ['packages/web/server/lib/opencode/skills.js', 'packages/web/server/lib/opencode/mcp.js', 'packages/ui/src/stores/usePluginsStore.ts'],
    testPaths: ['packages/web/server/lib/opencode/skills.test.js', 'packages/web/server/lib/opencode/mcp.test.js', 'packages/ui/src/stores/usePluginsStore.test.ts'],
  },
  {
    id: 'notifications-and-tts',
    sourcePaths: ['packages/web/server/lib/notifications/runtime.js', 'packages/web/server/lib/tts/routes.js'],
    testPaths: ['packages/web/server/lib/notifications/runtime.test.js', 'packages/web/server/lib/tts/routes.test.js'],
  },
  {
    id: 'browser-preview-and-session-scoped-agent-leases',
    sourcePaths: [
      'packages/web/server/lib/browser-cdp/discovery-runtime.js',
      'packages/web/server/lib/browser-cdp/lease-runtime.js',
      'packages/web/server/lib/browser-cdp/observation-runtime.js',
      'packages/web/server/lib/agent-browser/install.js',
      'packages/web/server/default-config/plugins/devryan-browser.mjs',
      'packages/electron/browser-cdp-bridge.mjs',
      'packages/electron/browser-surface-manager.mjs',
      'packages/electron/desktop-host-broker.mjs',
      'packages/electron/main.mjs',
      'packages/ui/src/lib/browserAgentApi.ts',
      'packages/ui/src/stores/useBrowserAgentStore.ts',
    ],
    testPaths: [
      'packages/web/server/lib/browser-cdp/discovery-runtime.test.js',
      'packages/web/server/lib/browser-cdp/lease-runtime.test.js',
      'packages/web/server/lib/browser-cdp/observation-runtime.test.js',
      'packages/web/server/lib/agent-browser/install.test.js',
      'packages/web/server/default-config/plugins/devryan-browser.test.mjs',
      'packages/electron/tests/browser-cdp-bridge.test.mjs',
      'packages/electron/tests/browser-lease-main-contract.test.mjs',
      'packages/electron/tests/browser-surface-main-contract.test.mjs',
      'packages/electron/tests/desktop-host-broker.test.mjs',
      'packages/ui/src/stores/useBrowserAgentStore.test.ts',
      'packages/ui/src/components/layout/browserGuestLifecycle.test.ts',
    ],
  },
  {
    id: 'product-tool-contracts',
    sourcePaths: [
      'packages/ui/src/lib/tools/manifest.ts',
      'packages/ui/src/lib/toolHelpers.ts',
      'packages/ui/src/components/chat/message/parts/tool-activity/classification.ts',
      'packages/ui/src/components/chat/ManagedTaskList.tsx',
    ],
    testPaths: [
      'packages/ui/src/lib/tools/manifest.test.ts',
      'packages/ui/src/lib/toolHelpers.test.ts',
      'packages/ui/src/components/chat/message/parts/toolRenderUtils.test.ts',
      'packages/ui/src/components/chat/message/parts/toolDisplayState.test.ts',
      'packages/ui/src/components/chat/message/parts/toolExpandedFallback.test.ts',
      'packages/ui/src/components/chat/message/parts/ToolPart.diff.test.ts',
      'packages/ui/src/components/chat/message/parts/taskToolUtils.test.ts',
      'packages/ui/src/components/chat/ManagedTaskList.test.tsx',
      'packages/web/src/api/tools.test.ts',
    ],
  },
  {
    id: 'provider-prompt-tool-policy',
    sourcePaths: ['packages/ui/src/lib/opencode/provider-prompt-tools.ts'],
    testPaths: ['packages/ui/src/lib/opencode/provider-prompt-tools.test.ts', 'packages/ui/src/lib/opencode/client.send.test.ts'],
  },
  {
    id: 'cli-tunnels-and-auth',
    sourcePaths: ['packages/web/bin/cli.js', 'packages/web/server/lib/tunnels/index.js'],
    testPaths: ['packages/web/bin/cli.test.js', 'packages/web/server/lib/tunnels/index.test.js'],
  },
  {
    id: 'packaging-and-release-branding',
    sourcePaths: ['.github/workflows/release.yml', 'scripts/verify-release-assets.mjs'],
    testPaths: ['scripts/verify-release-assets.test.mjs', 'scripts/verify-default-config-artifact.test.mjs'],
  },
  {
    id: 'validation-and-bundle-budgets',
    sourcePaths: ['scripts/validate.mjs', 'scripts/check-bundle-budgets.mjs'],
    testPaths: ['scripts/test-runner-utils.test.mjs', 'scripts/check-bundle-budgets.test.mjs'],
  },
  {
    id: 'agent-evaluation-tools',
    sourcePaths: ['scripts/agent-evals/main.mjs', 'scripts/agent-evals/runner.mjs'],
    testPaths: ['scripts/agent-evals/main.test.mjs', 'scripts/agent-evals/runner.test.mjs'],
  },
]);
