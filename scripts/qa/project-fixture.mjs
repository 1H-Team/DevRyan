...
export const createQaProjectPrompts = ({ agent = 'builder', planMode = false } = {}) => {
  ...
  const nativeVerification = 'Capture this exact causal repair sequence in separate completed tool calls: successfully read `src/tasks.mjs` with native read first; after that read completes, run standalone `node --test test/tasks.test.mjs` with native bash and observe the initial failure; only then edit implementation when authorized; after the final edit, run that standalone native bash command again and verify it passes. Do not run the initial test before or concurrently with the source read. Preserve the real numeric exit status of each test call. Do not wrap those commands in scripts or command chains. Already completed diagnosis need not be repeated after approval. Also run `npm test` for the full suite. ' + verificationScope;
