...
Paths may be absolute or repository-relative. `devRyanBaseUrl` must be credential-free HTTP(S) loopback with an empty path or `/api`. Case IDs are `inspect`, `repair-and-test`, `managed-change`, `oracle-review-focused`, and `oracle-review-deep`.
