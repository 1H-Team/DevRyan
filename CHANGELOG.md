...
- Fixed the app freezing for up to a minute after launch, and sessions stuck on their first tool call (for example "loading skills"), in projects with large gitignored folders. The 1.2.14 background ledger build walked gitignored trees such as build outputs, caches and virtual environments on the window's main thread and restarted on every launch.
  - Gitignored directories without tracked files are now dependency inputs, like `node_modules`: linked read-only into confined views and never copied into the ledger. Confined writes into them fail, and outputs written there are not published (a replaced link is reported as `ignoredInputs`). Gitignored single files such as `.env` are still available.
  - Revert, Redo and file restore leave paths inside such a directory untouched and report an `ignored_input` conflict.
  - The background build is a single pass capped at 20,000 eligible files or 512 MiB, is never treated as a real call's observation, and yields to the app between batches. Diffs of very different text are bounded (about 5 s to under 0.1 s for 16 KiB), with exact but coarser ownership past the budget.
  - Ledger maintenance and input-classification failures are journaled instead of silently ignored.
- Hidden models in Settings → Providers no longer reset after about 20 models: hidden model references are kept up to 2048 instead of 64, on save and on reload.
- The model picker groups current models by provider, keeps older models accessible in a separate section, and improves keyboard navigation. Retired Antigravity integration and its stale provider and quota entries are removed.
- Provider tabs now follow their display names, and the model picker no longer shows a time-based NEW badge.
- The built-in skill tool loads through the native read-only receipt path, so skill loading does not wait for a confined worker; the loading indicator clears when the completion receipt arrives.
- Saving settings logs changed key names instead of the full request body.

- DevRyan companion 2.1.0 on OpenCode 1.18.32:
  - Built-in read, glob and grep record one direct receipt (one lock-free admission and one commit) instead of the four-step lease protocol; a revert or cancellation in between discards the result, and a crash leaves the call `uncertain` as before (`DEVRYAN_DIRECT_CONTROL_RECEIPTS=0` restores the lease protocol).
  - A worker for a plugin-defined tool loads only its owning plugin plus plugins with worker-side hooks (`DEVRYAN_WORKER_OWNING_PLUGIN=0` loads every plugin).
  - Unmatched OpenCode paths answer 404 instead of being proxied to app.opencode.ai or served the web UI (`DEVRYAN_OPENCODE_UI_BLOCK=0` restores the upstream fallback).
  - Confined edits with OpenCode LSP enabled no longer rebuild language servers on every call. Each worker used to download vscode-eslint from GitHub, run its npm install and compile, and reinstall `typescript-language-server`. Workers now skip download-backed servers (`DEVRYAN_WORKER_LSP_DOWNLOAD=1` restores them) and link one host-installed `typescript-language-server` read-only (`DEVRYAN_WORKER_LSP_SHARED=0`). TypeScript diagnostics after an edit are unchanged; a TypeScript edit dropped from about 28 s to about 7.7 s before the shared link.
  - Provider recovery conformance verified on 1.18.32.
- Confined execution ledger on large projects (measured on a 5,677-file repository):
  - The first confined call drops from about 3 minutes to about 70 seconds, and a background build when a project opens removes that wait entirely (`DEVRYAN_LEDGER_PREWARM=0`).
  - Warm calls prepare in about 1.9 s instead of 6.4 s and publish in about 0.8 s instead of 3.6 s; a control call takes about 0.7 s instead of 3.5 s.
  - The ledger's Git store is packed in the background, and old unreachable objects are pruned, so it no longer grows unpacked forever (`DEVRYAN_LEDGER_PACK=0`). First builds skip empty listings and re-parsing (`DEVRYAN_LEDGER_FAST_INGEST=0`); parsed records are cached by tree identity (`DEVRYAN_LEDGER_RECORD_CACHE=0`, `DEVRYAN_LEDGER_SNAPSHOT_REUSE=0`); ancestor symlink checks are memoized per pass (`DEVRYAN_LEDGER_ANCESTOR_MEMO=0`).
- Duplicate outputs are qualified, and on by default, for four routes on companion 2.1.0: xAI `grok-4.7` Medium and `grok-4.6` High, and OpenAI `gpt-6-astra` Medium and `gpt-5.6-sol` Medium. Ten-pair live acceptance plus a Plan-mode instruction-reuse supplement passed on each, cutting primary-request provider input by 26% (xAI) and 29% (OpenAI) with unchanged outcomes. Claude through Meridian stays unqualified because its loopback traffic cannot be observed.
- Duplicate-output qualification binds to the companion build identity (upstream version, base commit, patch and build inputs) instead of an exact binary hash, so a rebuild of the same source on another machine keeps its qualification, and it records the request-shaping policy vector (`DEVRYAN_DUPLICATE_IDENTITY=binary` restores exact pinning). The v1.2.12 release binary differed from the qualified local build only by embedded build paths, so duplicate outputs were silently off in that release; the orchestration plugin change in this release marks the existing profile stale until requalification.
- Session changes no longer mark every confined edit as a receipt conflict. Besides the confined publication's byte-exact receipt, each edit also got receipts rebuilt from the edit tool's metadata (the plugin's after hook, canonical part events and history replay), which for OpenCode 1.18 carry a patch without file contents. They never matched, so coverage went `partial` (`receipt_conflict`), restore was reported unavailable, and every history replay re-notified the UI, which polled the changes endpoint continuously (about 1.6 requests per second per open session). A confined receipt is now the authority for its call: metadata receipts fill gaps only and never override or conflict with it, and a confined receipt replaces them (`DEVRYAN_CONFINED_RECEIPT_AUTHORITY=0` restores the comparison). Calls already flagged in existing ledgers keep their flag.
- Managed delegation:
  - `devryan_task` validates agent names against the project's live catalog before starting anything: `Fixer` is canonicalized to `fixer`; unknown or ambiguous names fail with `managed_agent_unknown` listing the available subagents; an unreadable catalog fails closed (`DEVRYAN_TASK_AGENT_VALIDATION=0` restores the old behaviour).
  - `wait_any` and its arguments are advertised only while `DEVRYAN_MANAGED_WAIT_ANY=1` (`DEVRYAN_CAPABILITY_TOOL_SCHEMA=0` restores the static schema).
- The bundled Open Cursor plugin no longer replaces OpenCode's own tools. open-cursor 2.5.8 registered `read`, `glob`, `bash`, `shell`, `ls`, `mkdir`, `rm`, `stat` and `oc_*` aliases for every agent. Plugin tools take precedence over same-named built-ins, and these never ask OpenCode for permission, so they bypassed `.env`, external-directory and bash command rules, and made every read a confined worker call (about 3.5 s) instead of a native read (about 0.1 s). A DevRyan adapter (`plugins/devryan-open-cursor.mjs`) now loads the installed package without those tools; existing profiles migrate automatically, and Cursor models still run through the Cursor SDK runtime or the proxy's OpenCode tool loop (`DEVRYAN_OPEN_CURSOR_LOCAL_TOOLS=1` restores the tools). The adapter also turns off open-cursor's MCP bridge, which connected every enabled MCP server again from each OpenCode instance, with config headers only, and added `mcp__*` duplicates of OpenCode's own MCP tools; an explicit `CURSOR_ACP_MCP_BRIDGE` is respected (`DEVRYAN_OPEN_CURSOR_MCP_BRIDGE=1` restores it).
- DevRyan no longer forwards unknown OpenCode API paths: the `/api` pass-through answers 404 (`opencode_route_unknown`) using OpenCode's live `/doc` route table with an SDK-generated fallback (`DEVRYAN_OPENCODE_ROUTE_GUARD=0`).
- Agent settings:
  - Settings shows the model that actually runs when a Slim entry sets only a variant.
  - `GET /api/config/agent-overrides` reports `shadowedOverrides`: saved sidecar model/variant values that Slim overrides, with the effective value (read-only; nothing is pruned).
- Tests and tooling: deterministic agent-eval client, scoped-revert and audit-outbox timing tests; a Claude Code 2.1.251 → 2.1.281 in-place upgrade test; `multi-session-sampler.mjs` gains `--pid` and `--runtime-root` targets with LSP spawn chains; new `scripts/perf/ledger-benchmark.mjs` and `scripts/qa/agent-visual-smoke.mjs`; duplicate-output qualification tooling selects OpenAI or xAI routes (host-attested `xai-oauth-responses-v1`) and waits for in-flight wire requests before deleting a trial session.
...
- Provider recovery: coordinate stalled primary-provider recovery in the host runtime across web and Electron.
...
- Release: bump the DevRyan workspace, Electron, web, runtime packages, and legacy desktop metadata to 1.1.8.
...
- Git and files: add shared runtime file caching, richer file API contracts, and root-aware Git/file operations across web.
- OpenCode: add managed process registries for web startup paths and improve managed lifecycle cleanup.
- CLI and agents: strengthen command coverage and refresh orchestrator guidance for delegated specialist workflows.

## [1.1.7] - 2026-06-25

- OpenCode: update the bundled SDK dependency and external runtime recommendation to 1.17.11 across web, UI, diagnostics.
...
- Git and files: add primary worktree root routing, richer file API contracts, and cross-runtime filesystem handling for web.
...
- OpenCode: update the bundled SDK dependency and external runtime recommendation to 1.17.10 across web, UI, diagnostics.

## [1.1.4] - 2026-06-21

- OpenCode: update the bundled SDK dependency and external runtime recommendation to 1.17.9 across web, UI, diagnostics.
...
- OpenCode: update the bundled SDK dependency and external runtime recommendation to 1.17.8 across web, UI, diagnostics.
