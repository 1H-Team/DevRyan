-- Browser-side crashes (client.error) join the three server-projected diagnostic
-- actions. The action column is free text, so only the impact-filter index, the
-- Error Logs clear RPC, and its retention escape hatch need the new action.

drop index if exists public.activity_logs_diagnostic_impact_created_idx;
...
begin
  ...
  where resolution.action in ('diagnostic.recovered', 'diagnostic.unresolved')
    ...
    and exists (
      ...
      where failure.event_id::text = resolution.target_id
        and failure.action in ('session.error', 'tool.failed', 'managed_task.failed', 'client.error')
  ...
  where failure.action in ('session.error', 'tool.failed', 'managed_task.failed', 'client.error')
    and failure.created_at <= p_until
    and (p_since is null or failure.created_at >= p_since);
...
comment on index public.activity_logs_diagnostic_impact_created_idx is
  'Impact-filtered Error Logs pagination across server-projected and browser-reported failures.';
