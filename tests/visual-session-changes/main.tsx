...
opencodeClient.getSessionChangesDiffPage = async (_session, _directory, _revision, file, cursor) => ({
...
function Fixture() {
    ...
    const files = (large ? Array.from({ length: Math.min(128, 513 - pageIndex * 128) }, (_, i) => `src/generated-${String(pageIndex * 128 + i).padStart(5, '0')}.ts`) : session === 'A' ? ['src/app.ts', 'src/theme.css', 'assets/icon.png', 'tests/app.test.ts'] : ['src/calendar.ts']).map((file, index) => ({
    ...
    const state = resolveSessionChangesFooterState({
        ...
        isRevertPending: false, isTreeWorking: working, isSiblingWorking: false,
    ...
    return <main style={{ maxWidth: mobile ? 390 : 900, margin: 'auto', padding: 24, minHeight: '100vh' }}>
        ...
        <p>{planning ? 'Plan presented' : working ? 'Implementation in progress' : 'Implementation completed'} for session {session}.</p>
        {state.visible && <SessionChangesCardView key={session} directory="/fixture" files={visibleFiles} subagentCount={session === 'A' ? 1 : 0}
            fileCount={large ? 513 : visibleFiles.length} pageIndex={pageIndex}
